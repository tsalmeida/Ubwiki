...
* @version 4.7.3
*/error_reporting(6135);$qc=!preg_match('~^(unsafe_raw)?$~',ini_get("filter.default"));if($qc||ini_get("filter.default_flags")){foreach(array('_GET','_POST','_COOKIE','_SERVER')as$X){$Ig=filter_input_array(constant("INPUT$X"),FILTER_UNSAFE_RAW);if($Ig)$$X=$Ig;}}if(function_exists("mb_internal_encoding"))mb_internal_encoding("8bit");function
connection(){global$h;return$h;}function
adminer(){global$b;return$b;}function
version(){global$ca;return$ca;}function
idf_unescape($u){$Ad=substr($u,-1);return
str_replace($Ad.$Ad,$Ad,substr($u,1,-1));}function
escape_string($X){return
substr(q($X),1,-1);}function
number($X){return
preg_replace('~[^0-9]+~','',$X);}function
number_type(){return'((?<!o)int(?!er)|numeric|real|float|double|decimal|money)';}function
remove_slashes($Re,$qc=false){if(get_magic_quotes_gpc()){while(list($y,$X)=each($Re)){foreach($X
as$rd=>$W){unset($Re[$y][$rd]);if(is_array($W)){$Re[$y][stripslashes($rd)]=$W;$Re[]=&$Re[$y][stripslashes($rd)];}else$Re[$y][stripslashes($rd)]=($qc?$W:stripslashes($W));}}}}function
bracket_escape($u,$Ia=false){static$ug=array(':'=>':1',']'=>':2','['=>':3','"'=>':4');return
strtr($u,($Ia?array_flip($ug):$ug));}function
min_version($Vg,$Md="",$i=null){global$h;if(!$i)$i=$h;$Af=$i->server_info;if($Md&&preg_match('~([\d.]+)-MariaDB~',$Af,$A)){$Af=$A[1];$Vg=$Md;}return(version_compare($Af,$Vg)>=0);}function
charset($h){return(min_version("5.5.3",0,$h)?"utf8mb4":"utf8");}function
script($Jf,$tg="\n"){return"<script".nonce().">$Jf</script>$tg";}function
script_src($Ng){return"<script src='".h($Ng)."'".nonce()."></script>\n";}function
...
nl_br($Q){return
str_replace("\n","<br>",$Q);}function
checkbox($B,$Y,$Xa,$xd="",$oe="",$bb="",$yd=""){$J="<input type='checkbox' name='$B' value='".h($Y)."'".($Xa?" checked":"").($yd?" aria-labelledby='$yd'":"").">".($oe?script("qsl('input').onclick = function () { $oe };",""):"");return($xd!=""||$bb?"<label".($bb?" class='$bb'":"").">$J".h($xd)."</label>":$J);}function
optionlist($C,$vf=null,$Rg=false){$J="";foreach($C
as$rd=>$W){$te=array($rd=>$W);if(is_array($W)){$J.='<optgroup label="'.h($rd).'">';$te=$W;}foreach($te
as$y=>$X)$J.='<option'.($Rg||is_string($y)?' value="'.h($y).'"':'').(($Rg||is_string($y)?(string)$y:$X)===$vf?' selected':'').'>'.h($X);if(is_array($W))$J.='</optgroup>';}return$J;}function
html_select($B,$C,$Y="",$ne=true,$yd=""){if($ne)return"<select name='".h($B)."'".($yd?" aria-labelledby='$yd'":"").">".optionlist($C,$Y)."</select>".(is_string($ne)?script("qsl('select').onchange = function () { $ne };",""):"");$J="";foreach($C
as$y=>$X)$J.="<label><input type='radio' name='".h($B)."' value='".h($y)."'".($y==$Y?" checked":"").">".h($X)."</label>";return$J;}function
select_input($Da,$C,$Y="",$ne="",$Ie=""){$cg=($C?"select":"input");return"<$cg$Da".($C?"><option value=''>$Ie".optionlist($C,$Y,true)."</select>":" size='10' value='".h($Y)."' placeholder='$Ie'>").($ne?script("qsl('$cg').onchange = $ne;",""):"");}function
confirm($Ud="",$wf="qsl('input')"){return
script("$wf.onclick = function () { return confirm('".($Ud?js_escape($Ud):lang(0))."'); };","");}function
print_fieldset($t,$Cd,$Yg=false){echo"<fieldset><legend>","<a href='#fieldset-$t'>$Cd</a>",script("qsl('a').onclick = partial(toggle, 'fieldset-$t');",""),"</legend>","<div id='fieldset-$t'".($Yg?"":" class='hidden'").">\n";}function
...
set_password($Ug,$O,$V,$F){$_SESSION["pwds"][$Ug][$O][$V]=($_COOKIE["adminer_key"]&&is_string($F)?array(encrypt_string($F,$_COOKIE["adminer_key"])):$F);}function
...
get_key_vals($G,$i=null,$Df=true){global$h;if(!is_object($i))$i=$h;$J=array();$I=$i->query($G);if(is_object($I)){while($K=$I->fetch_row()){if($Df)$J[$K[0]]=$K[1];else$J[]=$K[0];}}return$J;}function
...
cookie($B,$Y,$Fd=2592000){global$aa;return
header("Set-Cookie: $B=".urlencode($Y).($Fd?"; expires=".gmdate("D, d M Y H:i:s",time()+$Fd)." GMT":"")."; path=".preg_replace('~\?.*~','',$_SERVER["REQUEST_URI"]).($aa?"; secure":"")."; HttpOnly; SameSite=lax",false);}function
restart_session(){if(!ini_bool("session.use_cookies"))session_start();}function
stop_session($wc=false){$Qg=ini_bool("session.use_cookies");if(!$Qg||$wc){session_write_close();if($Qg&&@ini_set("session.use_cookies",false)===false)session_start();}}function&get_session($y){return$_SESSION[$y][DRIVER][SERVER][$_GET["username"]];}function
set_session($y,$X){$_SESSION[$y][DRIVER][SERVER][$_GET["username"]]=$X;}function
auth_url($Ug,$O,$V,$m=null){global$Jb;preg_match('~([^?]*)\??(.*)~',remove_from_uri(implode("|",array_keys($Jb))."|username|".($m!==null?"db|":"").session_name()),$A);return"$A[1]?".(sid()?SID."&":"").($Ug!="server"||$O!=""?urlencode($Ug)."=".urlencode($O)."&":"")."username=".urlencode($V).($m!=""?"&db=".urlencode($m):"").($A[2]?"&$A[2]":"");}function
...
query_redirect($G,$Hd,$Ud,$bf=true,$cc=true,$jc=false,$jg=""){global$h,$o,$b;if($cc){$Pf=microtime(true);$jc=!$h->query($G);$jg=format_time($Pf);}$Mf="";if($G)$Mf=$b->messageQuery($G,$jg,$jc);if($jc){$o=error().$Mf.script("messagesPrint();");return
false;}if($bf)redirect($Hd,$Ud.$Mf);return
true;}function
queries($G){global$h;static$Ve=array();static$Pf;if(!$Pf)$Pf=microtime(true);if($G===null)return
array(implode("\n",$Ve),format_time($Pf));$Ve[]=(preg_match('~;$~',$G)?"DELIMITER ;;\n$G;\nDELIMITER ":$G).";";return$h->query($G);}function
apply_queries($G,$T,$Zb='table'){foreach($T
as$R){if(!queries("$G ".$Zb($R)))return
false;}return
true;}function
queries_redirect($Hd,$Ud,$bf){list($Ve,$jg)=queries(null);return
query_redirect($Ve,$Hd,$Ud,$bf,false,!$bf,$jg);}function
...
null;foreach($oc
as$y=>$X)$oc[$y]=(array)$X;$J='';foreach($oc["error"]as$y=>$o){if($o)return$o;$B=$oc["name"][$y];$qg=$oc["tmp_name"][$y];$pb=file_get_contents($_b&&preg_match('~\.gz$~',$B)?"compress.zlib://$qg":$qg);if($_b){$Pf=substr($pb,0,3);if(function_exists("iconv")&&preg_match("~^\xFE\xFF|^\xFF\xFE~",$Pf,$cf))$pb=iconv("utf-16","utf-8",$pb);elseif($Pf=="\xEF\xBB\xBF")$pb=substr($pb,3);$J.=$pb."\n\n";}else$J.=$pb;}return$J;}function
upload_error($o){$Rd=($o==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($o?lang(2).($Rd?" ".lang(3,$Rd):""):lang(4));}function
repeat_pattern($Ge,$Dd){return
str_repeat("$Ge{0,65535}",$Dd/65535)."$Ge{0,".($Dd%65535)."}";}function
is_utf8($X){return(preg_match('~~u',$X)&&!preg_match('~[\0-\x8\xB\xC\xE-\x1F]~',$X));}function
shorten_utf8($Q,$Dd=80,$Wf=""){if(!preg_match("(^(".repeat_pattern("[\t\r\n -\x{10FFFF}]",$Dd).")($)?)u",$Q,$A))preg_match("(^(".repeat_pattern("[\t\r\n -~]",$Dd).")($)?)",$Q,$A);return
h($A[1]).$Wf.(isset($A[2])?"":"<i>â€¦</i>");}function
format_number($X){return
strtr(number_format($X,0,".",lang(5)),preg_split('~~u',lang(6),-1,PREG_SPLIT_NO_EMPTY));}function
friendly_url($X){return
preg_replace('~[^a-z0-9_]~i','-',$X);}function
hidden_fields($Re,$Zc=array()){$J=false;while(list($y,$X)=each($Re)){if(!in_array($y,$Zc)){if(is_array($X)){foreach($X
as$rd=>$W)$Re[$y."[$rd]"]=$W;}else{$J=true;echo'<input type="hidden" name="'.h($y).'" value="'.h($X).'">';}}}return$J;}function
...
enum_input($U,$Da,$p,$Y,$Ub=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$p["length"],$Od);$J=($Ub!==null?"<label><input type='$U'$Da value='$Ub'".((is_array($Y)?in_array($Ub,$Y):$Y===0)?" checked":"")."><i>".lang(7)."</i></label>":"");foreach($Od[1]as$s=>$X){$X=stripcslashes(str_replace("''","'",$X));$Xa=(is_int($Y)?$Y==$s+1:(is_array($Y)?in_array($s+1,$Y):$Y===$X));$J.=" <label><input type='$U'$Da value='".($s+1)."'".($Xa?' checked':'').'>'.h($b->editVal($X,$p)).'</label>';}return$J;}function
input($p,$Y,$Fc){global$Dg,$b,$x;$B=h(bracket_escape($p["field"]));echo"<td class='function'>";if(is_array($Y)&&!$Fc){$ya=array($Y);if(version_compare(PHP_VERSION,5.4)>=0)$ya[]=JSON_PRETTY_PRINT;$Y=call_user_func_array('json_encode',$ya);$Fc="json";}$hf=($x=="mssql"&&$p["auto_increment"]);if($hf&&!$_POST["save"])$Fc=null;$Gc=(isset($_GET["select"])||$hf?array("orig"=>lang(8)):array())+$b->editFunctions($p);$Da=" name='fields[$B]'";if($p["type"]=="enum")echo
h($Gc[""])."<td>".$b->editInput($_GET["edit"],$p,$Da,$Y);else{$Nc=(in_array($Fc,$Gc)||isset($Gc[$Fc]));echo(count($Gc)>1?"<select name='function[$B]'>".optionlist($Gc,$Fc===null||$Nc?$Fc:"")."</select>".on_help("getTarget(event).value.replace(/^SQL\$/, '')",1).script("qsl('select').onchange = functionChange;",""):h(reset($Gc))).'<td>';$kd=$b->editInput($_GET["edit"],$p,$Da,$Y);if($kd!="")echo$kd;elseif(preg_match('~bool~',$p["type"]))echo"<input type='hidden'$Da value='0'>"."<input type='checkbox'".(preg_match('~^(1|t|true|y|yes|on)$~i',$Y)?" checked='checked'":"")."$Da value='1'>";elseif($p["type"]=="set"){preg_match_all("~'((?:[^']|'')*)'~",$p["length"],$Od);foreach($Od[1]as$s=>$X){$X=stripcslashes(str_replace("''","'",$X));$Xa=(is_int($Y)?($Y>>$s)&1:in_array($X,explode(",",$Y),true));echo" <label><input type='checkbox' name='fields[$B][$s]' value='".(1<<$s)."'".($Xa?' checked':'').">".h($b->editVal($X,$p)).'</label>';}}elseif(preg_match('~blob|bytea|raw|file~',$p["type"])&&ini_bool("file_uploads"))echo"<input type='file' name='fields-$B'>";elseif(($gg=preg_match('~text|lob|memo~i',$p["type"]))||preg_match("~\n~",$Y)){if($gg&&$x!="sqlite")$Da.=" cols='50' rows='12'";else{$L=min(12,substr_count($Y,"\n")+1);$Da.=" cols='30' rows='$L'".($L==1?" style='height: 1.2em;'":"");}echo"<textarea$Da>".h($Y).'</textarea>';}elseif($Fc=="json"||preg_match('~^jsonb?$~',$p["type"]))echo"<textarea$Da cols='50' rows='12' class='jush-js'>".h($Y).'</textarea>';else{$Td=(!preg_match('~int~',$p["type"])&&preg_match('~^(\d+)(,(\d+))?$~',$p["length"],$A)?((preg_match("~binary~",$p["type"])?2:1)*$A[1]+($A[3]?1:0)+($A[2]&&!$p["unsigned"]?1:0)):($Dg[$p["type"]]?$Dg[$p["type"]]+($p["unsigned"]?0:1):0));if($x=='sql'&&min_version(5.6)&&preg_match('~time~',$p["type"]))$Td+=7;echo"<input".((!$Nc||$Fc==="")&&preg_match('~(?<!o)int(?!er)~',$p["type"])&&!preg_match('~\[\]~',$p["full_type"])?" type='number'":"")." value='".h($Y)."'".($Td?" data-maxlength='$Td'":"").(preg_match('~char|binary~',$p["type"])&&$Td>20?" size='40'":"")."$Da>";}echo$b->editHint($_GET["edit"],$p,$Y);$rc=0;foreach($Gc
...
search_tables(){global$b,$h;$_GET["where"][0]["val"]=$_POST["query"];$yf="<ul>\n";foreach(table_status('',true)as$R=>$S){$B=$b->tableName($S);if(isset($S["Engine"])&&$B!=""&&(!$_POST["tables"]||in_array($R,$_POST["tables"]))){$I=$h->query("SELECT".limit("1 FROM ".table($R)," WHERE ".implode(" AND ",$b->selectSearchProcess(fields($R),array())),1));if(!$I||$I->fetch_row()){$Pe="<a href='".h(ME."select=".urlencode($R)."&where[0][op]=".urlencode($_GET["where"][0]["op"])."&where[0][val]=".urlencode($_GET["where"][0]["val"]))."'>$B</a>";echo"$yf<li>".($I?$Pe:"<p class='error'>$Pe: ".error())."\n";$yf="";}}}echo($yf?"<p class='message'>".lang(9):"</ul>")."\n";}function
dump_headers($Wc,$Zd=false){global$b;$J=$b->dumpHeaders($Wc,$Zd);$ye=$_POST["output"];if($ye!="text")header("Content-Disposition: attachment; filename=".$b->dumpFilename($Wc).".$J".($ye!="file"&&!preg_match('~[^0-9a-z]~',$ye)?".$ye":""));session_write_close();ob_flush();flush();return$J;}function
...
select_value($X,$_,$p,$hg){global$b;if(is_array($X)){$J="";foreach($X
as$rd=>$W)$J.="<tr>".($X!=array_values($X)?"<th>".h($rd):"")."<td>".select_value($W,$_,$p,$hg);return"<table cellspacing='0'>$J</table>";}if(!$_)$_=$b->selectLink($X,$p);if($_===null){if(is_mail($X))$_="mailto:$X";if(is_url($X))$_=$X;}$J=$b->editVal($X,$p);if($J!==null){if(!is_utf8($J))$J="\0";elseif($hg!=""&&is_shortable($p))$J=shorten_utf8($J,max(0,+$hg));else$J=h($J);}return$b->selectVal($J,$_,$p,$X);}function
is_mail($Rb){$Aa='[-a-z0-9!#$%&\'*+/=?^_`{|}~]';$Ib='[a-z0-9]([-a-z0-9]{0,61}[a-z0-9])';$Ge="$Aa+(\\.$Aa+)*@($Ib?\\.)+$Ib";return
is_string($Rb)&&preg_match("(^$Ge(,\\s*$Ge)*\$)i",$Rb);}function
...
slow_query($G){global$b,$sg,$n;$m=$b->database();$kg=$b->queryTimeout();$Gf=$n->slowQuery($G,$kg);if(!$Gf&&support("kill")&&is_object($i=connect())&&($m==""||$i->select_db($m))){$wd=$i->result(connection_id());echo'<script',nonce(),'>
var timeout = setTimeout(function () {
	ajax(\'',js_escape(ME),'script=kill\', function () {
	}, \'kill=',$wd,'&token=',$sg,'\');
}, ',1000*$kg,');
</script>
';}else$i=null;ob_flush();flush();$J=@get_key_vals(($Gf?$Gf:$G),$i,false);if($i){echo
script("clearTimeout(timeout);");ob_flush();flush();}return$J;}function
get_token(){$Xe=rand(1,1e6);return($Xe^$_SESSION["token"]).":$Xe";}function
verify_token(){list($sg,$Xe)=explode(":",$_POST["token"]);return($Xe^$_SESSION["token"])==$sg;}function
lzw_decompress($Na){$Gb=256;$Oa=8;$db=array();$jf=0;$kf=0;for($s=0;$s<strlen($Na);$s++){$jf=($jf<<8)+ord($Na[$s]);$kf+=8;if($kf>=$Oa){$kf-=$Oa;$db[]=$jf>>$kf;$jf&=(1<<$kf)-1;$Gb++;if($Gb>>$Oa)$Oa++;}}$Fb=range("\0","\xFF");$J="";foreach($db
as$s=>$cb){$Qb=$Fb[$cb];if(!isset($Qb))$Qb=$hh.$hh[0];$J.=$Qb;if($s)$Fb[]=$hh.$Qb[0];$hh=$Qb;}return$J;}function
on_help($ib,$Ef=0){return
script("mixin(qsl('select, input'), {onmouseover: function (event) { helpMouseover.call(this, event, $ib, $Ef) }, onmouseout: helpMouseout});","");}function
edit_form($a,$q,$K,$Lg){global$b,$x,$sg,$o;$ag=$b->tableName(table_status1($a,true));page_header(($Lg?lang(10):lang(11)),$o,array("select"=>array($a,$ag)),$ag);if($K===false)echo"<p class='error'>".lang(12)."\n";echo'<form action="" method="post" enctype="multipart/form-data" id="form">
...
query($G,$Eg=false){$I=parent::query($G);$this->error="";if(!$I){list(,$this->errno,$this->error)=$this->errorInfo();if(!$this->error)$this->error=lang(21);return
...
select($R,$M,$Z,$Hc,$ue=array(),$z=1,$D=0,$Pe=false){global$b,$x;$pd=(count($Hc)<count($M));$G=$b->selectQueryBuild($M,$Z,$Hc,$ue,$z,$D);if(!$G)$G="SELECT".limit(($_GET["page"]!="last"&&$z!=""&&$Hc&&$pd&&$x=="sql"?"SQL_CALC_FOUND_ROWS ":"").implode(", ",$M)."\nFROM ".table($R),($Z?"\nWHERE ".implode(" AND ",$Z):"").($Hc&&$pd?"\nGROUP BY ".implode(", ",$Hc):"").($ue?"\nORDER BY ".implode(", ",$ue):""),($z!=""?+$z:null),($D?$z*$D:0),"\n");$Pf=microtime(true);$J=$this->_conn->query($G);if($Pe)echo$b->selectQuery($G,$Pf,!$J);return$J;}function
delete($R,$H,$z=0){$G="FROM ".table($R);return
queries("DELETE".($z?limit1($R,$G,$H):" $G$H"));}function
update($R,$P,$H,$z=0,$N="\n"){$Tg=array();foreach($P
as$y=>$X)$Tg[]="$y = $X";$G=table($R)." SET$N".implode(",$N",$Tg);return
queries("UPDATE".($z?limit1($R,$G,$H,$N):" $G$H"));}function
insert($R,$P){return
queries("INSERT INTO ".table($R).($P?" (".implode(", ",array_keys($P)).")\nVALUES (".implode(", ",$P).")":" DEFAULT VALUES"));}function
insertUpdate($R,$L,$Ne){return
false;}function
begin(){return
...
quoteBinary($pf){return
q($pf);}function
warnings(){return'';}function
tableHelp($B){}}$Jb["sqlite"]="SQLite 3";$Jb["sqlite2"]="SQLite 2";if(isset($_GET["sqlite"])||isset($_GET["sqlite2"])){$Le=array((isset($_GET["sqlite"])?"SQLite3":"SQLite"),"PDO_SQLite");define("DRIVER",(isset($_GET["sqlite"])?"sqlite":"sqlite2"));if(class_exists(isset($_GET["sqlite"])?"SQLite3":"SQLiteDatabase")){if(isset($_GET["sqlite"])){class
Min_SQLite{var$extension="SQLite3",$server_info,$affected_rows,$errno,$error,$_link;function
__construct($r){$this->_link=new
SQLite3($r);$Vg=$this->_link->version();$this->server_info=$Vg["versionString"];}function
...
query($G,$Eg=false){$Xd=($Eg?"unbufferedQuery":"query");$I=@$this->_link->$Xd($G,SQLITE_BOTH,$o);$this->error="";if(!$I){$this->error=$o;return
...
fetch_field(){$B=$this->_result->fieldName($this->_offset++);$Ge='(\[.*]|"(?:[^"]|"")*"|(.+))';if(preg_match("~^($Ge\\.)?$Ge\$~",$B,$A)){$R=($A[3]!=""?$A[3]:idf_unescape($A[2]));$B=($A[5]!=""?$A[5]:idf_unescape($A[4]));}return(object)array("name"=>$B,"orgname"=>$B,"orgtable"=>$R,);}}}}elseif(extension_loaded("pdo_sqlite")){class
Min_SQLite
extends
...
extends
Min_SQL{function
insertUpdate($R,$L,$Ne){$Tg=array();foreach($L
as$P)$Tg[]="(".implode(", ",$P).")";return
...
array();}function
limit($G,$Z,$z,$ke=0,$N=" "){return" $G$Z".($z!==null?$N."LIMIT $z".($ke?" OFFSET $ke":""):"");}function
...
fields($R){global$h;$J=array();$Ne="";foreach(get_rows("PRAGMA table_info(".table($R).")")as$K){$B=$K["name"];$U=strtolower($K["type"]);$Ab=$K["dflt_value"];$J[$B]=array("field"=>$B,"type"=>(preg_match('~int~i',$U)?"integer":(preg_match('~char|clob|text~i',$U)?"text":(preg_match('~blob~i',$U)?"blob":(preg_match('~real|floa|doub~i',$U)?"real":"numeric")))),"full_type"=>$U,"default"=>(preg_match("~'(.*)'~",$Ab,$A)?str_replace("''","'",$A[1]):($Ab=="NULL"?null:$Ab)),"null"=>!$K["notnull"],"privileges"=>array("select"=>1,"insert"=>1,"update"=>1),"primary"=>$K["pk"],);if($K["pk"]){if($Ne!="")$J[$Ne]["auto_increment"]=false;elseif(preg_match('~^integer$~i',$U))$J[$B]["auto_increment"]=true;$Ne=$B;}}$Mf=$h->result("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = ".q($R));preg_match_all('~(("[^"]*+")+|[a-z0-9_]+)\s+text\s+COLLATE\s+(\'[^\']+\'|\S+)~i',$Mf,$Od,PREG_SET_ORDER);foreach($Od
as$A){$B=str_replace('""','"',preg_replace('~^"|"$~','',$A[1]));if($J[$B])$J[$B]["collation"]=trim($A[3],"'");}return$J;}function
indexes($R,$i=null){global$h;if(!is_object($i))$i=$h;$J=array();$Mf=$i->result("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = ".q($R));if(preg_match('~\bPRIMARY\s+KEY\s*\((([^)"]+|"[^"]*"|`[^`]*`)++)~i',$Mf,$A)){$J[""]=array("type"=>"PRIMARY","columns"=>array(),"lengths"=>array(),"descs"=>array());preg_match_all('~((("[^"]*+")+|(?:`[^`]*+`)+)|(\S+))(\s+(ASC|DESC))?(,\s*|$)~i',$A[1],$Od,PREG_SET_ORDER);foreach($Od
as$A){$J[""]["columns"][]=idf_unescape($A[2]).$A[4];$J[""]["descs"][]=(preg_match('~DESC~i',$A[5])?'1':null);}}if(!$J){foreach(fields($R)as$B=>$p){if($p["primary"])$J[""]=array("type"=>"PRIMARY","columns"=>array($B),"lengths"=>array(),"descs"=>array(null));}}$Nf=get_key_vals("SELECT name, sql FROM sqlite_master WHERE type = 'index' AND tbl_name = ".q($R),$i);foreach(get_rows("PRAGMA index_list(".table($R).")",$i)as$K){$B=$K["name"];$v=array("type"=>($K["unique"]?"UNIQUE":"INDEX"));$v["lengths"]=array();$v["descs"]=array();foreach(get_rows("PRAGMA index_info(".idf_escape($B).")",$i)as$of){$v["columns"][]=$of["name"];$v["descs"][]=null;}if(preg_match('~^CREATE( UNIQUE)? INDEX '.preg_quote(idf_escape($B).' ON '.idf_escape($R),'~').' \((.*)\)$~i',$Nf[$B],$cf)){preg_match_all('/("[^"]*+")+( DESC)?/',$cf[2],$Od);foreach($Od[2]as$y=>$X){if($X)$v["descs"][$y]='1';}}if(!$J[""]||$v["type"]!="UNIQUE"||$v["columns"]!=$J[""]["columns"]||$v["descs"]!=$J[""]["descs"]||!preg_match("~^sqlite_~",$B))$J[$B]=$v;}return$J;}function
...
alter_table($R,$B,$q,$xc,$jb,$Vb,$d,$Ga,$De){global$h;$Pg=($R==""||$xc);foreach($q
as$p){if($p[0]!=""||!$p[1]||$p[2]){$Pg=true;break;}}$c=array();$xe=array();foreach($q
as$p){if($p[1]){$c[]=($Pg?$p[1]:"ADD ".implode($p[1]));if($p[0]!="")$xe[$p[0]]=$p[1][0];}}if(!$Pg){foreach($c
as$X){if(!queries("ALTER TABLE ".table($R)." $X"))return
false;}if($R!=$B&&!queries("ALTER TABLE ".table($R)." RENAME TO ".table($B)))return
false;}elseif(!recreate_table($R,$B,$c,$xe,$xc,$Ga))return
...
true;}function
recreate_table($R,$B,$q,$xe,$xc,$Ga,$w=array()){global$h;if($R!=""){if(!$q){foreach(fields($R)as$y=>$p){if($w)$p["auto_increment"]=0;$q[]=process_field($p,$p);$xe[$y]=idf_escape($y);}}$Oe=false;foreach($q
as$p){if($p[6])$Oe=true;}$Lb=array();foreach($w
as$y=>$X){if($X[2]=="DROP"){$Lb[$X[1]]=true;unset($w[$y]);}}foreach(indexes($R)as$ud=>$v){$f=array();foreach($v["columns"]as$y=>$e){if(!$xe[$e])continue
2;$f[]=$xe[$e].($v["descs"][$y]?" DESC":"");}if(!$Lb[$ud]){if($v["type"]!="PRIMARY"||!$Oe)$w[]=array($v["type"],$ud,$f);}}foreach($w
as$y=>$X){if($X[0]=="PRIMARY"){unset($w[$y]);$xc[]="  PRIMARY KEY (".implode(", ",$X[2]).")";}}foreach(foreign_keys($R)as$ud=>$_c){foreach($_c["source"]as$y=>$e){if(!$xe[$e])continue
2;$_c["source"][$y]=idf_unescape($xe[$e]);}if(!isset($xc[" $ud"]))$xc[]=" ".format_foreign_key($_c);}queries("BEGIN");}foreach($q
as$y=>$p)$q[$y]="  ".implode($p);$q=array_merge($q,array_filter($xc));$eg=($R==$B?"adminer_$B":$B);if(!queries("CREATE TABLE ".table($eg)." (\n".implode(",\n",$q)."\n)"))return
false;if($R!=""){if($xe&&!queries("INSERT INTO ".table($eg)." (".implode(", ",$xe).") SELECT ".implode(", ",array_map('idf_escape',array_keys($xe)))." FROM ".table($R)))return
false;$Bg=array();foreach(triggers($R)as$_g=>$lg){$zg=trigger($_g);$Bg[]="CREATE TRIGGER ".idf_escape($_g)." ".implode(" ",$lg)." ON ".table($B)."\n$zg[Statement]";}$Ga=$Ga?0:$h->result("SELECT seq FROM sqlite_sequence WHERE name = ".q($R));if(!queries("DROP TABLE ".table($R))||($R==$B&&!queries("ALTER TABLE ".table($eg)." RENAME TO ".table($B)))||!alter_indexes($B,$w))return
false;if($Ga)queries("UPDATE sqlite_sequence SET seq = $Ga WHERE name = ".q($B));foreach($Bg
...
move_tables($T,$Xg,$dg){return
false;}function
trigger($B){global$h;if($B=="")return
array("Statement"=>"BEGIN\n\t;\nEND");$u='(?:[^`"\s]+|`[^`]*`|"[^"]*")+';$Ag=trigger_options();preg_match("~^CREATE\\s+TRIGGER\\s*$u\\s*(".implode("|",$Ag["Timing"]).")\\s+([a-z]+)(?:\\s+OF\\s+($u))?\\s+ON\\s*$u\\s*(?:FOR\\s+EACH\\s+ROW\\s)?(.*)~is",$h->result("SELECT sql FROM sqlite_master WHERE type = 'trigger' AND name = ".q($B)),$A);$je=$A[3];return
array("Timing"=>strtoupper($A[1]),"Event"=>strtoupper($A[2]).($je?" OF":""),"Of"=>($je[0]=='`'||$je[0]=='"'?idf_unescape($je):$je),"Trigger"=>$B,"Statement"=>$A[4],);}function
triggers($R){$J=array();$Ag=trigger_options();foreach(get_rows("SELECT * FROM sqlite_master WHERE type = 'trigger' AND tbl_name = ".q($R))as$K){preg_match('~^CREATE\s+TRIGGER\s*(?:[^`"\s]+|`[^`]*`|"[^"]*")+\s*('.implode("|",$Ag["Timing"]).')\s*(.*?)\s+ON\b~i',$K["sql"],$A);$J[$K["name"]]=array($A[1],$A[2]);}return$J;}function
...
true;}function
create_sql($R,$Ga,$Uf){global$h;$J=$h->result("SELECT sql FROM sqlite_master WHERE type IN ('table', 'view') AND name = ".q($R));foreach(indexes($R)as$B=>$v){if($B=='')continue;$J.=";\n\n".index_sql($R,$v['type'],$B,"(".implode(", ",array_map('idf_escape',$v['columns'])).")");}return$J;}function
...
show_status(){$J=array();foreach(get_vals("PRAGMA compile_options")as$se){list($y,$X)=explode("=",$se,2);$J[$y]=$X;}return$J;}function
convert_field($p){}function
unconvert_field($p,$J){return$J;}function
support($lc){return
preg_match('~^(columns|database|drop_col|dump|indexes|descidx|move_col|sql|status|table|trigger|variables|view|view_trigger)$~',$lc);}$x="sqlite";$Dg=array("integer"=>0,"real"=>0,"numeric"=>0,"text"=>0,"blob"=>0);$Tf=array_keys($Dg);$Kg=array();$re=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT IN","IS NOT NULL","SQL");$Gc=array("hex","length","lower","round","unixepoch","upper");$Kc=array("avg","count","count distinct","group_concat","max","min","sum");$Ob=array(array(),array("integer|real|numeric"=>"+/-","text"=>"||",));}$Jb["pgsql"]="PostgreSQL";if(isset($_GET["pgsql"])){$Le=array("PgSQL","PDO_PgSQL");define("DRIVER","pgsql");if(extension_loaded("pgsql")){class
...
query($G,$Eg=false){$I=@pg_query($this->_link,$G);$this->error="";if(!$I){$this->error=pg_last_error($this->_link);$J=false;}elseif(!pg_num_fields($I)){$this->affected_rows=pg_affected_rows($I);$J=true;}else$J=new
...
quoteBinary($pf){return
q($pf);}function
query($G,$Eg=false){$J=parent::query($G,$Eg);if($this->timeout){$this->timeout=0;parent::query("RESET statement_timeout");}return$J;}function
warnings(){return'';}function
close(){}}}class
Min_Driver
extends
Min_SQL{function
insertUpdate($R,$L,$Ne){global$h;foreach($L
as$P){$Lg=array();$Z=array();foreach($P
as$y=>$X){$Lg[]="$y = $X";if(isset($Ne[idf_unescape($y)]))$Z[]="$y = $X";}if(!(($Z&&queries("UPDATE ".table($R)." SET ".implode(", ",$Lg)." WHERE ".implode(" AND ",$Z))&&$h->affected_rows)||queries("INSERT INTO ".table($R)." (".implode(", ",array_keys($P)).") VALUES (".implode(", ",$P).")")))return
false;}return
true;}function
slowQuery($G,$kg){$this->_conn->query("SET statement_timeout = ".(1000*$kg));$this->_conn->timeout=1000*$kg;return$G;}function
...
quoteBinary($pf){return$this->_conn->quoteBinary($pf);}function
warnings(){return$this->_conn->warnings();}function
tableHelp($B){$Gd=array("information_schema"=>"infoschema","pg_catalog"=>"catalog",);$_=$Gd[$_GET["ns"]];if($_)return"$_-".str_replace("_","-",$B).".html";}}function
idf_escape($u){return'"'.str_replace('"','""',$u).'"';}function
table($u){return
idf_escape($u);}function
connect(){global$b,$Dg,$Tf;$h=new
Min_DB;$j=$b->credentials();if($h->connect($j[0],$j[1],$j[2])){if(min_version(9,0,$h)){$h->query("SET application_name = 'Adminer'");if(min_version(9.2,0,$h)){$Tf[lang(25)][]="json";$Dg["json"]=4294967295;if(min_version(9.4,0,$h)){$Tf[lang(25)][]="jsonb";$Dg["jsonb"]=4294967295;}}}return$h;}return$h->error;}function
get_databases(){return
...
fk_support($S){return
true;}function
fields($R){$J=array();$xa=array('timestamp without time zone'=>'timestamp','timestamp with time zone'=>'timestamptz',);$Xc=min_version(10)?"(a.attidentity = 'd')::int":'0';foreach(get_rows("SELECT a.attname AS field, format_type(a.atttypid, a.atttypmod) AS full_type, d.adsrc AS default, a.attnotnull::int, col_description(c.oid, a.attnum) AS comment, $Xc AS identity
FROM pg_class c
...
AND a.attnum > 0
ORDER BY a.attnum")as$K){preg_match('~([^([]+)(\((.*)\))?([a-z ]+)?((\[[0-9]*])*)$~',$K["full_type"],$A);list(,$U,$Dd,$K["length"],$sa,$za)=$A;$K["length"].=$za;$Wa=$U.$sa;if(isset($xa[$Wa])){$K["type"]=$xa[$Wa];$K["full_type"]=$K["type"].$Dd.$za;}else{$K["type"]=$U;$K["full_type"]=$K["type"].$Dd.$sa.$za;}if($K['identity'])$K['default']='GENERATED BY DEFAULT AS IDENTITY';$K["null"]=!$K["attnotnull"];$K["auto_increment"]=$K['identity']||preg_match('~^nextval\(~i',$K["default"]);$K["privileges"]=array("insert"=>1,"select"=>1,"update"=>1);if(preg_match('~(.+)::[^)]+(.*)~',$K["default"],$A))$K["default"]=($A[1]=="NULL"?null:(($A[1][0]=="'"?idf_unescape($A[1]):$A[1]).$A[2]));$J[$K["field"]]=$K;}return$J;}function
indexes($R,$i=null){global$h;if(!is_object($i))$i=$h;$J=array();$bg=$i->result("SELECT oid FROM pg_class WHERE relnamespace = (SELECT oid FROM pg_namespace WHERE nspname = current_schema()) AND relname = ".q($R));$f=get_key_vals("SELECT attnum, attname FROM pg_attribute WHERE attrelid = $bg AND attnum > 0",$i);foreach(get_rows("SELECT relname, indisunique::int, indisprimary::int, indkey, indoption , (indpred IS NOT NULL)::int as indispartial FROM pg_index i, pg_class ci WHERE i.indrelid = $bg AND ci.oid = i.indexrelid",$i)as$K){$df=$K["relname"];$J[$df]["type"]=($K["indispartial"]?"INDEX":($K["indisprimary"]?"PRIMARY":($K["indisunique"]?"UNIQUE":"INDEX")));$J[$df]["columns"]=array();foreach(explode(" ",$K["indkey"])as$fd)$J[$df]["columns"][]=$f[$fd];$J[$df]["descs"]=array();foreach(explode(" ",$K["indoption"])as$gd)$J[$df]["descs"][]=($gd&1?'1':null);$J[$df]["lengths"]=array();}return$J;}function
foreign_keys($R){global$me;$J=array();foreach(get_rows("SELECT conname, condeferrable::int AS deferrable, pg_get_constraintdef(oid) AS definition
FROM pg_constraint
WHERE conrelid = (SELECT pc.oid FROM pg_class AS pc INNER JOIN pg_namespace AS pn ON (pn.oid = pc.relnamespace) WHERE pc.relname = ".q($R)." AND pn.nspname = current_schema())
AND contype = 'f'::char
ORDER BY conkey, conname")as$K){if(preg_match('~FOREIGN KEY\s*\((.+)\)\s*REFERENCES (.+)\((.+)\)(.*)$~iA',$K['definition'],$A)){$K['source']=array_map('trim',explode(',',$A[1]));if(preg_match('~^(("([^"]|"")+"|[^"]+)\.)?"?("([^"]|"")+"|[^"]+)$~',$A[2],$Nd)){$K['ns']=str_replace('""','"',preg_replace('~^"(.+)"$~','\1',$Nd[2]));$K['table']=str_replace('""','"',preg_replace('~^"(.+)"$~','\1',$Nd[4]));}$K['target']=array_map('trim',explode(',',$A[3]));$K['on_delete']=(preg_match("~ON DELETE ($me)~",$A[4],$Nd)?$Nd[1]:'NO ACTION');$K['on_update']=(preg_match("~ON UPDATE ($me)~",$A[4],$Nd)?$Nd[1]:'NO ACTION');$J[$K['conname']]=$K;}}return$J;}function
view($B){global$h;return
...
alter_table($R,$B,$q,$xc,$jb,$Vb,$d,$Ga,$De){$c=array();$Ve=array();if($R!=""&&$R!=$B)$Ve[]="ALTER TABLE ".table($R)." RENAME TO ".table($B);foreach($q
as$p){$e=idf_escape($p[0]);$X=$p[1];if(!$X)$c[]="DROP $e";else{$Sg=$X[5];unset($X[5]);if(isset($X[6])&&$p[0]=="")$X[1]=($X[1]=="bigint"?" big":" ")."serial";if($p[0]=="")$c[]=($R!=""?"ADD ":"  ").implode($X);else{if($e!=$X[0])$Ve[]="ALTER TABLE ".table($B)." RENAME $e TO $X[0]";$c[]="ALTER $e TYPE$X[1]";if(!$X[6]){$c[]="ALTER $e ".($X[3]?"SET$X[3]":"DROP DEFAULT");$c[]="ALTER $e ".($X[2]==" NULL"?"DROP NOT":"SET").$X[2];}}if($p[0]!=""||$Sg!="")$Ve[]="COMMENT ON COLUMN ".table($B).".$X[0] IS ".($Sg!=""?substr($Sg,9):"''");}}$c=array_merge($c,$xc);if($R=="")array_unshift($Ve,"CREATE TABLE ".table($B)." (\n".implode(",\n",$c)."\n)");elseif($c)array_unshift($Ve,"ALTER TABLE ".table($R)."\n".implode(",\n",$c));if($R!=""||$jb!="")$Ve[]="COMMENT ON TABLE ".table($B)." IS ".q($jb);if($Ga!=""){}foreach($Ve
as$G){if(!queries($G))return
false;}return
true;}function
alter_indexes($R,$c){$sb=array();$Kb=array();$Ve=array();foreach($c
as$X){if($X[0]!="INDEX")$sb[]=($X[2]=="DROP"?"\nDROP CONSTRAINT ".idf_escape($X[1]):"\nADD".($X[1]!=""?" CONSTRAINT ".idf_escape($X[1]):"")." $X[0] ".($X[0]=="PRIMARY"?"KEY ":"")."(".implode(", ",$X[2]).")");elseif($X[2]=="DROP")$Kb[]=idf_escape($X[1]);else$Ve[]="CREATE INDEX ".idf_escape($X[1]!=""?$X[1]:uniqid($R."_"))." ON ".table($R)." (".implode(", ",$X[2]).")";}if($sb)array_unshift($Ve,"ALTER TABLE ".table($R).implode(",",$sb));if($Kb)array_unshift($Ve,"DROP INDEX ".implode(", ",$Kb));foreach($Ve
as$G){if(!queries($G))return
false;}return
true;}function
truncate_tables($T){return
...
as$R){$Rf=table_status($R);if(!queries("DROP ".strtoupper($Rf["Engine"])." ".table($R)))return
false;}return
true;}function
move_tables($T,$Xg,$dg){foreach(array_merge($T,$Xg)as$R){$Rf=table_status($R);if(!queries("ALTER ".strtoupper($Rf["Engine"])." ".table($R)." SET SCHEMA ".idf_escape($dg)))return
false;}return
true;}function
...
found_rows($S,$Z){global$h;if(preg_match("~ rows=([0-9]+)~",$h->result("EXPLAIN SELECT * FROM ".idf_escape($S["Name"]).($Z?" WHERE ".implode(" AND ",$Z):"")),$cf))return$cf[1];return
false;}function
types(){return
...
set_schema($qf){global$h,$Dg,$Tf;$J=$h->query("SET search_path TO ".idf_escape($qf));foreach(types()as$U){if(!isset($Dg[$U])){$Dg[$U]=0;$Tf[lang(26)][]=$U;}}return$J;}function
create_sql($R,$Ga,$Uf){global$h;$J='';$mf=array();$_f=array();$Rf=table_status($R);$q=fields($R);$w=indexes($R);ksort($w);$uc=foreign_keys($R);ksort($uc);if(!$Rf||empty($q))return
false;$J="CREATE TABLE ".idf_escape($Rf['nspname']).".".idf_escape($Rf['Name'])." (\n    ";foreach($q
as$mc=>$p){$Ce=idf_escape($p['field']).' '.$p['full_type'].default_value($p).($p['attnotnull']?" NOT NULL":"");$mf[]=$Ce;if(preg_match('~nextval\(\'([^\']+)\'\)~',$p['default'],$Od)){$zf=$Od[1];$Lf=reset(get_rows(min_version(10)?"SELECT *, cache_size AS cache_value FROM pg_sequences WHERE schemaname = current_schema() AND sequencename = ".q($zf):"SELECT * FROM $zf"));$_f[]=($Uf=="DROP+CREATE"?"DROP SEQUENCE IF EXISTS $zf;\n":"")."CREATE SEQUENCE $zf INCREMENT $Lf[increment_by] MINVALUE $Lf[min_value] MAXVALUE $Lf[max_value] START ".($Ga?$Lf['last_value']:1)." CACHE $Lf[cache_value];";}}if(!empty($_f))$J=implode("\n\n",$_f)."\n\n$J";foreach($w
as$ad=>$v){switch($v['type']){case'UNIQUE':$mf[]="CONSTRAINT ".idf_escape($ad)." UNIQUE (".implode(', ',array_map('idf_escape',$v['columns'])).")";break;case'PRIMARY':$mf[]="CONSTRAINT ".idf_escape($ad)." PRIMARY KEY (".implode(', ',array_map('idf_escape',$v['columns'])).")";break;}}foreach($uc
as$tc=>$sc)$mf[]="CONSTRAINT ".idf_escape($tc)." $sc[definition] ".($sc['deferrable']?'DEFERRABLE':'NOT DEFERRABLE');$J.=implode(",\n    ",$mf)."\n) WITH (oids = ".($Rf['Oid']?'true':'false').");";foreach($w
as$ad=>$v){if($v['type']=='INDEX'){$f=array();foreach($v['columns']as$y=>$X)$f[]=idf_escape($X).($v['descs'][$y]?" DESC":"");$J.="\n\nCREATE INDEX ".idf_escape($ad)." ON ".idf_escape($Rf['nspname']).".".idf_escape($Rf['Name'])." USING btree (".implode(', ',$f).");";}}if($Rf['Comment'])$J.="\n\nCOMMENT ON TABLE ".idf_escape($Rf['nspname']).".".idf_escape($Rf['Name'])." IS ".q($Rf['Comment']).";";foreach($q
as$mc=>$p){if($p['comment'])$J.="\n\nCOMMENT ON COLUMN ".idf_escape($Rf['nspname']).".".idf_escape($Rf['Name']).".".idf_escape($mc)." IS ".q($p['comment']).";";}return
rtrim($J,';');}function
truncate_sql($R){return"TRUNCATE ".table($R);}function
trigger_sql($R){$Rf=table_status($R);$J="";foreach(triggers($R)as$yg=>$xg){$zg=trigger($yg,$Rf['Name']);$J.="\nCREATE TRIGGER ".idf_escape($zg['Trigger'])." $zg[Timing] $zg[Events] ON ".idf_escape($Rf["nspname"]).".".idf_escape($Rf['Name'])." $zg[Type] $zg[Statement];;\n";}return$J;}function
...
select_db($k){return
true;}function
query($G,$Eg=false){$I=oci_parse($this->_link,$G);$this->error="";if(!$I){$o=oci_error($this->_link);$this->errno=$o["code"];$this->error=$o["message"];return
...
limit($G,$Z,$z,$ke=0,$N=" "){return($ke?" * FROM (SELECT t.*, rownum AS rnum FROM (SELECT $G$Z) t WHERE rownum <= ".($z+$ke).") WHERE rnum > $ke":($z!==null?" * FROM (SELECT $G$Z) WHERE rownum <= ".($z+$ke):" $G$Z"));}function
limit1($R,$G,$Z,$N="\n"){return" $G$Z";}function
...
count_tables($l){return
array();}function
table_status($B=""){$J=array();$sf=q($B);foreach(get_rows('SELECT table_name "Name", \'table\' "Engine", avg_row_len * num_rows "Data_length", num_rows "Rows" FROM all_tables WHERE tablespace_name = '.q(DB).($B!=""?" AND table_name = $sf":"")."
UNION SELECT view_name, 'view', 0, 0 FROM user_views".($B!=""?" WHERE view_name = $sf":"")."
ORDER BY 1")as$K){if($B!="")return$K;$J[$K["Name"]]=$K;}return$J;}function
is_view($S){return$S["Engine"]=="view";}function
fk_support($S){return
true;}function
fields($R){$J=array();foreach(get_rows("SELECT * FROM all_tab_columns WHERE table_name = ".q($R)." ORDER BY column_id")as$K){$U=$K["DATA_TYPE"];$Dd="$K[DATA_PRECISION],$K[DATA_SCALE]";if($Dd==",")$Dd=$K["DATA_LENGTH"];$J[$K["COLUMN_NAME"]]=array("field"=>$K["COLUMN_NAME"],"full_type"=>$U.($Dd?"($Dd)":""),"type"=>strtolower($U),"length"=>$Dd,"default"=>$K["DATA_DEFAULT"],"null"=>($K["NULLABLE"]=="Y"),"privileges"=>array("insert"=>1,"select"=>1,"update"=>1),);}return$J;}function
...
found_rows($S,$Z){}function
alter_table($R,$B,$q,$xc,$jb,$Vb,$d,$Ga,$De){$c=$Kb=array();foreach($q
...
set_schema($rf){global$h;return$h->query("ALTER SESSION SET CURRENT_SCHEMA = ".idf_escape($rf));}function
show_variables(){return
...
unconvert_field($p,$J){return$J;}function
support($lc){return
preg_match('~^(columns|database|drop_col|indexes|descidx|processlist|scheme|sql|status|table|variables|view|view_trigger)$~',$lc);}$x="oracle";$Dg=array();$Tf=array();foreach(array(lang(27)=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),lang(28)=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),lang(25)=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),lang(29)=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$y=>$X){$Dg+=$X;$Tf[$y]=array_keys($X);}$Kg=array();$re=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","SQL");$Gc=array("length","lower","round","upper");$Kc=array("avg","count","count distinct","max","min","sum");$Ob=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$Jb["mssql"]="MS SQL (beta)";if(isset($_GET["mssql"])){$Le=array("SQLSRV","MSSQL","PDO_DBLIB");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
query($G,$Eg=false){$I=sqlsrv_query($this->_link,$G);$this->error="";if(!$I){$this->_get_error();return
...
query($G,$Eg=false){$I=@mssql_query($G,$this->_link);$this->error="";if(!$I){$this->error=mssql_get_last_message();return
...
extends
Min_SQL{function
insertUpdate($R,$L,$Ne){foreach($L
as$P){$Lg=array();$Z=array();foreach($P
as$y=>$X){$Lg[]="$y = $X";if(isset($Ne[idf_unescape($y)]))$Z[]="$y = $X";}if(!queries("MERGE ".table($R)." USING (VALUES(".implode(", ",$P).")) AS source (c".implode(", c",range(1,count($P))).") ON ".implode(" AND ",$Z)." WHEN MATCHED THEN UPDATE SET ".implode(", ",$Lg)." WHEN NOT MATCHED THEN INSERT (".implode(", ",array_keys($P)).") VALUES (".implode(", ",$P).");"))return
false;}return
true;}function
begin(){return
...
limit($G,$Z,$z,$ke=0,$N=" "){return($z!==null?" TOP (".($z+$ke).")":"")." $G$Z";}function
limit1($R,$G,$Z,$N="\n"){return
...
WHERE o.schema_id = SCHEMA_ID(".q(get_schema()).") AND o.type IN ('S', 'U', 'V') AND o.name = ".q($R))as$K){$U=$K["type"];$Dd=(preg_match("~char|binary~",$U)?$K["max_length"]:($U=="decimal"?"$K[precision],$K[scale]":""));$J[$K["name"]]=array("field"=>$K["name"],"full_type"=>$U.($Dd?"($Dd)":""),"type"=>$U,"length"=>$Dd,"default"=>$K["default"],"null"=>$K["is_nullable"],"auto_increment"=>$K["is_identity"],"collation"=>$K["collation_name"],"privileges"=>array("insert"=>1,"select"=>1,"update"=>1),"primary"=>$K["is_identity"],"comment"=>$kb[$K["name"]],);}return$J;}function
...
alter_table($R,$B,$q,$xc,$jb,$Vb,$d,$Ga,$De){$c=array();$kb=array();foreach($q
...
move_tables($T,$Xg,$dg){return
apply_queries("ALTER SCHEMA ".idf_escape($dg)." TRANSFER",array_merge($T,$Xg));}function
...
unconvert_field($p,$J){return$J;}function
support($lc){return
preg_match('~^(comment|columns|database|drop_col|indexes|descidx|scheme|sql|table|trigger|view|view_trigger)$~',$lc);}$x="mssql";$Dg=array();$Tf=array();foreach(array(lang(27)=>array("tinyint"=>3,"smallint"=>5,"int"=>10,"bigint"=>20,"bit"=>1,"decimal"=>0,"real"=>12,"float"=>53,"smallmoney"=>10,"money"=>20),lang(28)=>array("date"=>10,"smalldatetime"=>19,"datetime"=>19,"datetime2"=>19,"time"=>8,"datetimeoffset"=>10),lang(25)=>array("char"=>8000,"varchar"=>8000,"text"=>2147483647,"nchar"=>4000,"nvarchar"=>4000,"ntext"=>1073741823),lang(29)=>array("binary"=>8000,"varbinary"=>8000,"image"=>2147483647),)as$y=>$X){$Dg+=$X;$Tf[$y]=array_keys($X);}$Kg=array();$re=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT IN","IS NOT NULL");$Gc=array("len","lower","round","upper");$Kc=array("avg","count","count distinct","max","min","sum");$Ob=array(array("date|time"=>"getdate",),array("int|decimal|real|float|money|datetime"=>"+/-","char|text"=>"+",));}$Jb['firebird']='Firebird (alpha)';if(isset($_GET["firebird"])){$Le=array("interbase");define("DRIVER","firebird");if(extension_loaded("interbase")){class
Min_DB{var$extension="Firebird",$server_info,$affected_rows,$errno,$error,$_link,$_result;function
connect($O,$V,$F){$this->_link=ibase_connect($O,$V,$F);if($this->_link){$Og=explode(':',$O);$this->service_link=ibase_service_attach($Og[0],$V,$F);$this->server_info=ibase_server_info($this->service_link,IBASE_SVC_SERVER_VERSION);}else{$this->errno=ibase_errcode();$this->error=ibase_errmsg();}return(bool)$this->_link;}function
quote($Q){return"'".str_replace("'","''",$Q)."'";}function
select_db($k){return($k=="domain");}function
query($G,$Eg=false){$I=ibase_query($G,$this->_link);if(!$I){$this->errno=ibase_errcode();$this->error=ibase_errmsg();return
...
limit($G,$Z,$z,$ke=0,$N=" "){$J='';$J.=($z!==null?$N."FIRST $z".($ke?" SKIP $ke":""):"");$J.=" $G$Z";return$J;}function
limit1($R,$G,$Z,$N="\n"){return
limit($G,$Z,1,0,$N);}function
...
true;}function
support($lc){return
preg_match("~^(columns|sql|status|table)$~",$lc);}$x="firebird";$re=array("=");$Gc=array();$Kc=array();$Ob=array();}$Jb["simpledb"]="SimpleDB";if(isset($_GET["simpledb"])){$Le=array("SimpleXML + allow_url_fopen");define("DRIVER","simpledb");if(class_exists('SimpleXMLElement')&&ini_bool('allow_url_fopen')){class
Min_DB{var$extension="SimpleXML",$server_info='2009-04-15',$error,$timeout,$next,$affected_rows,$_result;function
select_db($k){return($k=="domain");}function
query($G,$Eg=false){$E=array('SelectExpression'=>$G,'ConsistentRead'=>'true');if($this->next)$E['NextToken']=$this->next;$I=sdb_request_all('Select','Item',$E,$this->timeout);$this->timeout=0;if($I===false)return$I;if(preg_match('~^\s*SELECT\s+COUNT\(~i',$G)){$Xf=0;foreach($I
as$qd)$Xf+=$qd->Attribute->Value;$I=array((object)array('Attribute'=>array((object)array('Name'=>'Count','Value'=>$Xf,))));}return
new
Min_Result($I);}function
multi_query($G){return$this->_result=$this->query($G);}function
store_result(){return$this->_result;}function
...
_chunkRequest($Yc,$ra,$E,$dc=array()){global$h;foreach(array_chunk($Yc,25)as$Za){$Be=$E;foreach($Za
as$s=>$t){$Be["Item.$s.ItemName"]=$t;foreach($dc
as$y=>$X)$Be["Item.$s.$y"]=$X;}if(!sdb_request($ra,$Be))return
false;}$h->affected_rows=count($Yc);return
true;}function
_extractIds($R,$H,$z){$J=array();if(preg_match_all("~itemName\(\) = (('[^']*+')+)~",$H,$Od))$J=array_map('idf_unescape',$Od[1]);else{foreach(sdb_request_all('Select','Item',array('SelectExpression'=>'SELECT itemName() FROM '.table($R).$H.($z?" LIMIT 1":"")))as$qd)$J[]=$qd->Name;}return$J;}function
select($R,$M,$Z,$Hc,$ue=array(),$z=1,$D=0,$Pe=false){global$h;$h->next=$_GET["next"];$J=parent::select($R,$M,$Z,$Hc,$ue,$z,$D,$Pe);$h->next=0;return$J;}function
...
as$rd=>$W){$ld["Attribute.$s.Name"]=$y;$ld["Attribute.$s.Value"]=(is_array($X)?$W:idf_unescape($W));if(!$rd)$ld["Attribute.$s.Replace"]="true";$s++;}}}$E=array('DomainName'=>$R);return(!$ld||$this->_chunkRequest(($t!=""?array($t):$Yc),'BatchPutAttributes',$E,$ld))&&(!$Bb||$this->_chunkRequest($Yc,'BatchDeleteAttributes',$E,$Bb));}function
...
rollback(){return
false;}function
slowQuery($G,$kg){$this->_conn->timeout=$kg;return$G;}}function
connect(){global$b;list(,,$F)=$b->credentials();if($F!="")return
lang(22);return
new
Min_DB;}function
...
table_status($B="",$kc=false){$J=array();foreach(($B!=""?array($B=>true):tables_list())as$R=>$U){$K=array("Name"=>$R,"Auto_increment"=>"");if(!$kc){$Wd=sdb_request('DomainMetadata',array('DomainName'=>$R));if($Wd){foreach(array("Rows"=>"ItemCount","Data_length"=>"ItemNamesSizeBytes","Index_length"=>"AttributeValuesSizeBytes","Data_free"=>"AttributeNamesSizeBytes",)as$y=>$X)$K[$y]=(string)$Wd->$X;}}if($B!="")return$K;$J[$R]=$K;}return$J;}function
...
idf_escape($u);}function
idf_escape($u){return"`".str_replace("`","``",$u)."`";}function
limit($G,$Z,$z,$ke=0,$N=" "){return" $G$Z".($z!==null?$N."LIMIT $z":"");}function
unconvert_field($p,$J){return$J;}function
fk_support($S){}function
engines(){return
array();}function
alter_table($R,$B,$q,$xc,$jb,$Vb,$d,$Ga,$De){return($R==""&&sdb_request('CreateDomain',array('DomainName'=>$B)));}function
drop_tables($T){foreach($T
...
last_id(){}function
hmac($wa,$xb,$y,$Ze=false){$Pa=64;if(strlen($y)>$Pa)$y=pack("H*",$wa($y));$y=str_pad($y,$Pa,"\0");$sd=$y^str_repeat("\x36",$Pa);$td=$y^str_repeat("\x5C",$Pa);$J=$wa($td.pack("H*",$wa($sd.$xb)));if($Ze)$J=pack("H*",$J);return$J;}function
sdb_request($ra,$E=array()){global$b,$h;list($Uc,$E['AWSAccessKeyId'],$tf)=$b->credentials();$E['Action']=$ra;$E['Timestamp']=gmdate('Y-m-d\TH:i:s+00:00');$E['Version']='2009-04-15';$E['SignatureVersion']=2;$E['SignatureMethod']='HmacSHA1';ksort($E);$G='';foreach($E
as$y=>$X)$G.='&'.rawurlencode($y).'='.rawurlencode($X);$G=str_replace('%7E','~',substr($G,1));$G.="&Signature=".urlencode(base64_encode(hmac('sha1',"POST\n".preg_replace('~^https?://~','',$Uc)."\n/\n$G",$tf,true)));@ini_set('track_errors',1);$oc=@file_get_contents((preg_match('~^https?://~',$Uc)?$Uc:"http://$Uc"),false,stream_context_create(array('http'=>array('method'=>'POST','content'=>$G,'ignore_errors'=>1,))));if(!$oc){$h->error=$php_errormsg;return
false;}libxml_use_internal_errors(true);$ih=simplexml_load_string($oc);if(!$ih){$o=libxml_get_last_error();$h->error=$o->message;return
false;}if($ih->Errors){$o=$ih->Errors->Error;$h->error="$o->Message ($o->Code)";return
false;}$h->error='';$cg=$ra."Result";return($ih->$cg?$ih->$cg:true);}function
sdb_request_all($ra,$cg,$E=array(),$kg=0){$J=array();$Pf=($kg?microtime(true):0);$z=(preg_match('~LIMIT\s+(\d+)\s*$~i',$E['SelectExpression'],$A)?$A[1]:0);do{$ih=sdb_request($ra,$E);if(!$ih)break;foreach($ih->$cg
as$Qb)$J[]=$Qb;if($z&&count($J)>=$z){$_GET["next"]=$ih->NextToken;break;}if($kg&&microtime(true)-$Pf>$kg)return
false;$E['NextToken']=$ih->NextToken;if($z)$E['SelectExpression']=preg_replace('~\d+\s*$~',$z-count($J),$E['SelectExpression']);}while($ih->NextToken);return$J;}$x="simpledb";$re=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","IS NOT NULL");$Gc=array();$Kc=array("count");$Ob=array(array("json"));}$Jb["mongo"]="MongoDB";if(isset($_GET["mongo"])){$Le=array("mongo","mongodb");define("DRIVER","mongo");if(class_exists('MongoDB')){class
Min_DB{var$extension="Mongo",$server_info=MongoClient::VERSION,$error,$last_id,$_link,$_db;function
connect($Mg,$C){return@new
MongoClient($Mg,$C);}function
query($G){return
false;}function
...
fetch_row(){$J=$this->fetch_assoc();if(!$J)return$J;return
array_values($J);}function
fetch_field(){$vd=array_keys($this->_rows[0]);$B=$vd[$this->_offset++];return(object)array('name'=>$B,'charsetnr'=>$this->_charset[$B],);}}class
Min_Driver
extends
Min_SQL{public$Ne="_id";function
select($R,$M,$Z,$Hc,$ue=array(),$z=1,$D=0,$Pe=false){$M=($M==array("*")?array():array_fill_keys($M,true));$If=array();foreach($ue
as$X){$X=preg_replace('~ DESC$~','',$X,1,$rb);$If[$X]=($rb?-1:1);}return
new
Min_Result($this->_conn->_db->selectCollection($R)->find(array(),$M)->sort($If)->limit($z!=""?+$z:0)->skip($D*$z));}function
...
as$m){$if=$h->_link->selectDB($m)->drop();if(!$if['ok'])return
false;}return
true;}function
...
fields($R){return
fields_from_edit();}function
found_rows($S,$Z){global$h;return$h->_db->selectCollection($_GET["select"])->count($Z);}$re=array("=");}elseif(class_exists('MongoDB\Driver\Manager')){class
Min_DB{var$extension="MongoDB",$server_info=MONGODB_VERSION,$error,$last_id;var$_link;var$_db,$_db_name;function
connect($Mg,$C){$bb='MongoDB\Driver\Manager';return
new$bb($Mg,$C);}function
query($G){return
false;}function
...
extends
Min_SQL{public$Ne="_id";function
select($R,$M,$Z,$Hc,$ue=array(),$z=1,$D=0,$Pe=false){global$h;$M=($M==array("*")?array():array_fill_keys($M,1));if(count($M)&&!isset($M['_id']))$M['_id']=0;$Z=where_to_query($Z);$If=array();foreach($ue
as$X){$X=preg_replace('~ DESC$~','',$X,1,$rb);$If[$X]=($rb?-1:1);}if(isset($_GET['limit'])&&is_numeric($_GET['limit'])&&$_GET['limit']>0)$z=$_GET['limit'];$z=min(200,max(1,(int)$z));$Ff=$D*$z;$bb='MongoDB\Driver\Query';$G=new$bb($Z,array('projection'=>$M,'limit'=>$z,'skip'=>$Ff,'sort'=>$If));$lf=$h->_link->executeQuery("$h->_db_name.$R",$G);return
new
Min_Result($lf);}function
update($R,$P,$H,$z=0,$N="\n"){global$h;$m=$h->_db_name;$Z=sql_query_where_parser($H);$bb='MongoDB\Driver\BulkWrite';$Ta=new$bb(array());if(isset($P['_id']))unset($P['_id']);$ef=array();foreach($P
as$y=>$Y){if($Y=='NULL'){$ef[$y]=1;unset($P[$y]);}}$Lg=array('$set'=>$P);if(count($ef))$Lg['$unset']=$ef;$Ta->update($Z,$Lg,array('upsert'=>false));$lf=$h->_link->executeBulkWrite("$m.$R",$Ta);$h->affected_rows=$lf->getModifiedCount();return
true;}function
delete($R,$H,$z=0){global$h;$m=$h->_db_name;$Z=sql_query_where_parser($H);$bb='MongoDB\Driver\BulkWrite';$Ta=new$bb(array());$Ta->delete($Z,array('limit'=>$z));$lf=$h->_link->executeBulkWrite("$m.$R",$Ta);$h->affected_rows=$lf->getDeletedCount();return
true;}function
insert($R,$P){global$h;$m=$h->_db_name;$bb='MongoDB\Driver\BulkWrite';$Ta=new$bb(array());if(isset($P['_id'])&&empty($P['_id']))unset($P['_id']);$Ta->insert($P);$lf=$h->_link->executeBulkWrite("$m.$R",$Ta);$h->affected_rows=$lf->getInsertedCount();return
true;}}function
get_databases($vc){global$h;$J=array();$bb='MongoDB\Driver\Command';$ib=new$bb(array('listDatabases'=>1));$lf=$h->_link->executeCommand('admin',$ib);foreach($lf
as$zb){foreach($zb->databases
as$m)$J[]=$m->name;}return$J;}function
count_tables($l){$J=array();return$J;}function
tables_list(){global$h;$bb='MongoDB\Driver\Command';$ib=new$bb(array('listCollections'=>1));$lf=$h->_link->executeCommand($h->_db_name,$ib);$gb=array();foreach($lf
as$I)$gb[$I->name]='table';return$gb;}function
drop_databases($l){return
false;}function
indexes($R,$i=null){global$h;$J=array();$bb='MongoDB\Driver\Command';$ib=new$bb(array('listIndexes'=>$R));$lf=$h->_link->executeCommand($h->_db_name,$ib);foreach($lf
...
found_rows($S,$Z){global$h;$Z=where_to_query($Z);$bb='MongoDB\Driver\Command';$ib=new$bb(array('count'=>$S['Name'],'query'=>$Z));$lf=$h->_link->executeCommand($h->_db_name,$ib);$rg=$lf->toArray();return$rg[0]->n;}function
sql_query_where_parser($H){$H=trim(preg_replace('/WHERE[\s]?[(]?\(?/','',$H));$H=preg_replace('/\)\)\)$/',')',$H);$fh=explode(' AND ',$H);$gh=explode(') OR (',$H);$Z=array();foreach($fh
as$dh)$Z[]=trim($dh);if(count($gh)==1)$gh=array();elseif(count($gh)>1)$Z=array();return
where_to_query($Z,$gh);}function
where_to_query($bh=array(),$ch=array()){global$b;$xb=array();foreach(array('and'=>$bh,'or'=>$ch)as$U=>$Z){if(is_array($Z)){foreach($Z
as$ec){list($eb,$pe,$X)=explode(" ",$ec,3);if($eb=="_id"){$X=str_replace('MongoDB\BSON\ObjectID("',"",$X);$X=str_replace('")',"",$X);$bb='MongoDB\BSON\ObjectID';$X=new$bb($X);}if(!in_array($pe,$b->operators))continue;if(preg_match('~^\(f\)(.+)~',$pe,$A)){$X=(float)$X;$pe=$A[1];}elseif(preg_match('~^\(date\)(.+)~',$pe,$A)){$yb=new
DateTime($X);$bb='MongoDB\BSON\UTCDatetime';$X=new$bb($yb->getTimestamp()*1000);$pe=$A[1];}switch($pe){case'=':$pe='$eq';break;case'!=':$pe='$ne';break;case'>':$pe='$gt';break;case'<':$pe='$lt';break;case'>=':$pe='$gte';break;case'<=':$pe='$lte';break;case'regex':$pe='$regex';break;default:continue
2;}if($U=='and')$xb['$and'][]=array($eb=>array($pe=>$X));elseif($U=='or')$xb['$or'][]=array($eb=>array($pe=>$X));}}}return$xb;}$re=array("=","!=",">","<",">=","<=","regex","(f)=","(f)!=","(f)>","(f)<","(f)>=","(f)<=","(date)=","(date)!=","(date)>","(date)<","(date)>=","(date)<=",);}function
...
engines(){return
array();}function
alter_table($R,$B,$q,$xc,$jb,$Vb,$d,$Ga,$De){global$h;if($R==""){$h->_db->createCollection($B);return
true;}}function
drop_tables($T){global$h;foreach($T
as$R){$if=$h->_db->selectCollection($R)->drop();if(!$if['ok'])return
false;}return
true;}function
truncate_tables($T){global$h;foreach($T
as$R){$if=$h->_db->selectCollection($R)->remove();if(!$if['ok'])return
false;}return
true;}$x="mongo";$Gc=array();$Kc=array();$Ob=array(array("json"));}$Jb["elastic"]="Elasticsearch (beta)";if(isset($_GET["elastic"])){$Le=array("json + allow_url_fopen");define("DRIVER","elastic");if(function_exists('json_decode')&&ini_bool('allow_url_fopen')){class
Min_DB{var$extension="JSON",$server_info,$errno,$error,$_url;function
rootQuery($Fe,$pb=array(),$Xd='GET'){@ini_set('track_errors',1);$oc=@file_get_contents("$this->_url/".ltrim($Fe,'/'),false,stream_context_create(array('http'=>array('method'=>$Xd,'content'=>$pb===null?$pb:json_encode($pb),'header'=>'Content-Type: application/json','ignore_errors'=>1,))));if(!$oc){$this->error=$php_errormsg;return$oc;}if(!preg_match('~^HTTP/[0-9.]+ 2~i',$http_response_header[0])){$this->error=$oc;return
...
query($Fe,$pb=array(),$Xd='GET'){return$this->rootQuery(($this->_db!=""?"$this->_db/":"/").ltrim($Fe,'/'),$pb,$Xd);}function
...
extends
Min_SQL{function
select($R,$M,$Z,$Hc,$ue=array(),$z=1,$D=0,$Pe=false){global$b;$xb=array();$G="$R/_search";if($M!=array("*"))$xb["fields"]=$M;if($ue){$If=array();foreach($ue
as$eb){$eb=preg_replace('~ DESC$~','',$eb,1,$rb);$If[]=($rb?array($eb=>"desc"):$eb);}$xb["sort"]=$If;}if($z){$xb["size"]=+$z;if($D)$xb["from"]=($D*$z);}foreach($Z
as$X){list($eb,$pe,$X)=explode(" ",$X,3);if($eb=="_id")$xb["query"]["ids"]["values"][]=$X;elseif($eb.$X!=""){$fg=array("term"=>array(($eb!=""?$eb:"_all")=>$X));if($pe=="=")$xb["query"]["filtered"]["filter"]["and"][]=$fg;else$xb["query"]["filtered"]["query"]["bool"]["must"][]=$fg;}}if($xb["query"]&&!$xb["query"]["filtered"]["query"]&&!$xb["query"]["ids"])$xb["query"]["filtered"]["query"]=array("match_all"=>array());$Pf=microtime(true);$sf=$this->_conn->query($G,$xb);if($Pe)echo$b->selectQuery("$G: ".json_encode($xb),$Pf,!$sf);if(!$sf)return
false;$J=array();foreach($sf['hits']['hits']as$Tc){$K=array();if($M==array("*"))$K["_id"]=$Tc["_id"];$q=$Tc['_source'];if($M!=array("*")){$q=array();foreach($M
as$y)$q[$y]=$Tc['fields'][$y];}foreach($q
as$y=>$X){if($xb["fields"])$X=$X[0];$K[$y]=(is_array($X)?json_encode($X):$X);}$J[]=$K;}return
new
Min_Result($J);}function
update($U,$af,$H,$z=0,$N="\n"){$Ee=preg_split('~ *= *~',$H);if(count($Ee)==2){$t=trim($Ee[1]);$G="$U/$t";return$this->_conn->query($G,$af,'POST');}return
false;}function
insert($U,$af){$t="";$G="$U/$t";$if=$this->_conn->query($G,$af,'POST');$this->_conn->last_id=$if['_id'];return$if['created'];}function
delete($U,$H,$z=0){$Yc=array();if(is_array($_GET["where"])&&$_GET["where"]["_id"])$Yc[]=$_GET["where"]["_id"];if(is_array($_POST['check'])){foreach($_POST['check']as$Va){$Ee=preg_split('~ *= *~',$Va);if(count($Ee)==2)$Yc[]=trim($Ee[1]);}}$this->_conn->affected_rows=0;foreach($Yc
as$t){$G="{$U}/{$t}";$if=$this->_conn->query($G,'{}','DELETE');if(is_array($if)&&$if['found']==true)$this->_conn->affected_rows++;}return$this->_conn->affected_rows;}}function
connect(){global$b;$h=new
...
as$dd=>$Qf){$cd=$Qf['total']['indexing'];$J[$dd]=$cd['index_total'];}}return$J;}function
tables_list(){global$h;$J=$h->query('_mapping');if($J)$J=array_fill_keys(array_keys($J[$h->_db]["mappings"]),'table');return$J;}function
table_status($B="",$kc=false){global$h;$sf=$h->query("_search",array("size"=>0,"aggregations"=>array("count_by_type"=>array("terms"=>array("field"=>"_type")))),"POST");$J=array();if($sf){$T=$sf["aggregations"]["count_by_type"]["buckets"];foreach($T
...
fields($R){global$h;$I=$h->query("$R/_mapping");$J=array();if($I){$Kd=$I[$R]['properties'];if(!$Kd)$Kd=$I[$h->_db]['mappings'][$R]['properties'];if($Kd){foreach($Kd
...
alter_table($R,$B,$q,$xc,$jb,$Vb,$d,$Ga,$De){global$h;$Se=array();foreach($q
as$ic){$mc=trim($ic[1][0]);$nc=trim($ic[1][1]?$ic[1][1]:"text");$Se[$mc]=array('type'=>$nc);}if(!empty($Se))$Se=array('properties'=>$Se);return$h->query("_mapping/{$B}",$Se,'PUT');}function
drop_tables($T){global$h;$J=true;foreach($T
as$R)$J=$J&&$h->query(urlencode($R),array(),'DELETE');return$J;}function
last_id(){global$h;return$h->last_id;}$x="elastic";$re=array("=","query");$Gc=array();$Kc=array();$Ob=array(array("json"));$Dg=array();$Tf=array();foreach(array(lang(27)=>array("long"=>3,"integer"=>5,"short"=>8,"byte"=>10,"double"=>20,"float"=>66,"half_float"=>12,"scaled_float"=>21),lang(28)=>array("date"=>10),lang(25)=>array("string"=>65535,"text"=>65535),lang(29)=>array("binary"=>255),)as$y=>$X){$Dg+=$X;$Tf[$y]=array_keys($X);}}$Jb["clickhouse"]="ClickHouse (alpha)";if(isset($_GET["clickhouse"])){define("DRIVER","clickhouse");class
...
false;}$J=json_decode($oc,true);if($J===null){$this->errno=json_last_error();if(function_exists('json_last_error_msg'))$this->error=json_last_error_msg();else{$ob=get_defined_constants(true);foreach($ob['json']as$B=>$Y){if($Y==$this->errno&&preg_match('~^JSON_ERROR_~',$B)){$this->error=$B;break;}}}}return
new
Min_Result($J);}function
...
update($R,$P,$H,$z=0,$N="\n"){$Tg=array();foreach($P
as$y=>$X)$Tg[]="$y = $X";$G=$N.implode(",$N",$Tg);return
...
alter_table($R,$B,$q,$xc,$jb,$Vb,$d,$Ga,$De){foreach($q
...
limit($G,$Z,$z,$ke=0,$N=" "){return" $G$Z".($z!==null?$N."LIMIT $z".($ke?", $ke":""):"");}function
limit1($R,$G,$Z,$N="\n"){return
limit($G,$Z,1,0,$N);}function
db_collation($m,$fb){}function
engines(){return
...
as$K){$U=trim($K['type']);$ge=strpos($U,'Nullable(')===0;$J[trim($K['name'])]=array("field"=>trim($K['name']),"full_type"=>$U,"type"=>$U,"default"=>trim($K['default_expression']),"null"=>$ge,"auto_increment"=>'0',"privileges"=>array("insert"=>1,"select"=>1,"update"=>0),);}return$J;}function
indexes($R,$i=null){return
array();}function
foreign_keys($R){return
array();}function
...
0;}function
support($lc){return
preg_match("~^(columns|sql|status|table)$~",$lc);}$x="clickhouse";$Dg=array();$Tf=array();foreach(array(lang(27)=>array("Int8"=>3,"Int16"=>5,"Int32"=>10,"Int64"=>19,"UInt8"=>3,"UInt16"=>5,"UInt32"=>10,"UInt64"=>20,"Float32"=>7,"Float64"=>16,'Decimal'=>38,'Decimal32'=>9,'Decimal64'=>18,'Decimal128'=>38),lang(28)=>array("Date"=>13,"DateTime"=>20),lang(25)=>array("String"=>0),lang(29)=>array("FixedString"=>0),)as$y=>$X){$Dg+=$X;$Tf[$y]=array_keys($X);}$Kg=array();$re=array("=","<",">","<=",">=","!=","~","!~","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT IN","IS NOT NULL","SQL");$Gc=array();$Kc=array("avg","count","count distinct","max","min","sum");$Ob=array();}$Jb=array("server"=>"MySQL")+$Jb;if(!defined("DRIVER")){$Le=array("MySQLi","MySQL","PDO_MySQL");define("DRIVER","server");if(extension_loaded("mysqli")){class
Min_DB
extends
MySQLi{var$extension="MySQLi";function
__construct(){parent::init();}function
connect($O="",$V="",$F="",$k=null,$Je=null,$Hf=null){global$b;mysqli_report(MYSQLI_REPORT_OFF);list($Uc,$Je)=explode(":",$O,2);$Of=$b->connectSsl();if($Of)$this->ssl_set($Of['key'],$Of['cert'],$Of['ca'],'','');$J=@$this->real_connect(($O!=""?$Uc:ini_get("mysqli.default_host")),($O.$V!=""?$V:ini_get("mysqli.default_user")),($O.$V.$F!=""?$F:ini_get("mysqli.default_pw")),$k,(is_numeric($Je)?$Je:ini_get("mysqli.default_port")),(!is_numeric($Je)?$Je:$Hf),($Of?64:0));$this->options(MYSQLI_OPT_LOCAL_INFILE,false);return$J;}function
...
query($G,$Eg=false){$I=@($Eg?mysql_unbuffered_query($G,$this->_link):mysql_query($G,$this->_link));$this->error="";if(!$I){$this->errno=mysql_errno($this->_link);$this->error=mysql_error($this->_link);return
...
connect($O,$V,$F){global$b;$C=array(PDO::MYSQL_ATTR_LOCAL_INFILE=>false);$Of=$b->connectSsl();if($Of){if(!empty($Of['key']))$C[PDO::MYSQL_ATTR_SSL_KEY]=$Of['key'];if(!empty($Of['cert']))$C[PDO::MYSQL_ATTR_SSL_CERT]=$Of['cert'];if(!empty($Of['ca']))$C[PDO::MYSQL_ATTR_SSL_CA]=$Of['ca'];}$this->dsn("mysql:charset=utf8;host=".str_replace(":",";unix_socket=",preg_replace('~:(\d)~',';port=\1',$O)),$V,$F,$C);return
true;}function
set_charset($Ua){$this->query("SET NAMES $Ua");}function
select_db($k){return$this->query("USE ".idf_escape($k));}function
query($G,$Eg=false){$this->setAttribute(1000,!$Eg);return
parent::query($G,$Eg);}}}class
Min_Driver
extends
Min_SQL{function
insert($R,$P){return($P?parent::insert($R,$P):queries("INSERT INTO ".table($R)." ()\nVALUES ()"));}function
insertUpdate($R,$L,$Ne){$f=array_keys(reset($L));$Me="INSERT INTO ".table($R)." (".implode(", ",$f).") VALUES\n";$Tg=array();foreach($f
as$y)$Tg[$y]="$y = VALUES($y)";$Wf="\nON DUPLICATE KEY UPDATE ".implode(", ",$Tg);$Tg=array();$Dd=0;foreach($L
as$P){$Y="(".implode(", ",$P).")";if($Tg&&(strlen($Me)+$Dd+strlen($Y)+strlen($Wf)>1e6)){if(!queries($Me.implode(",\n",$Tg).$Wf))return
false;$Tg=array();$Dd=0;}$Tg[]=$Y;$Dd+=strlen($Y)+2;}return
queries($Me.implode(",\n",$Tg).$Wf);}function
slowQuery($G,$kg){if(min_version('5.7.8','10.1.2')){if(preg_match('~MariaDB~',$this->_conn->server_info))return"SET STATEMENT max_statement_time=$kg FOR $G";elseif(preg_match('~^(SELECT\b)(.+)~is',$G,$A))return"$A[1] /*+ MAX_EXECUTION_TIME(".($kg*1000).") */ $A[2]";}}function
...
idf_escape($u);}function
connect(){global$b,$Dg,$Tf;$h=new
Min_DB;$j=$b->credentials();if($h->connect($j[0],$j[1],$j[2])){$h->set_charset(charset($h));$h->query("SET sql_quote_show_create = 1, autocommit = 1");if(min_version('5.7.8',10.2,$h)){$Tf[lang(25)][]="json";$Dg["json"]=4294967295;}return$h;}$J=$h->error;if(function_exists('iconv')&&!is_utf8($J)&&strlen($pf=iconv("windows-1250","utf-8",$J))>strlen($J))$J=$pf;return$J;}function
...
foreign_keys($R){global$h,$me;static$Ge='(?:`(?:[^`]|``)+`|"(?:[^"]|"")+")';$J=array();$tb=$h->result("SHOW CREATE TABLE ".table($R),1);if($tb){preg_match_all("~CONSTRAINT ($Ge) FOREIGN KEY ?\\(((?:$Ge,? ?)+)\\) REFERENCES ($Ge)(?:\\.($Ge))? \\(((?:$Ge,? ?)+)\\)(?: ON DELETE ($me))?(?: ON UPDATE ($me))?~",$tb,$Od,PREG_SET_ORDER);foreach($Od
as$A){preg_match_all("~$Ge~",$A[2],$Jf);preg_match_all("~$Ge~",$A[5],$dg);$J[idf_unescape($A[1])]=array("db"=>idf_unescape($A[4]!=""?$A[3]:$A[4]),"table"=>idf_unescape($A[4]!=""?$A[4]:$A[3]),"source"=>array_map('idf_unescape',$Jf[0]),"target"=>array_map('idf_unescape',$dg[0]),"on_delete"=>($A[6]?$A[6]:"RESTRICT"),"on_update"=>($A[7]?$A[7]:"RESTRICT"),);}}return$J;}function
view($B){global$h;return
...
rename_database($B,$d){$J=false;if(create_database($B,$d)){$ff=array();foreach(tables_list()as$R=>$U)$ff[]=table($R)." TO ".idf_escape($B).".".table($R);$J=(!$ff||queries("RENAME TABLE ".implode(", ",$ff)));if($J)queries("DROP DATABASE ".idf_escape(DB));restart_session();set_session("dbs",null);}return$J;}function
...
alter_table($R,$B,$q,$xc,$jb,$Vb,$d,$Ga,$De){$c=array();foreach($q
as$p)$c[]=($p[1]?($R!=""?($p[0]!=""?"CHANGE ".idf_escape($p[0]):"ADD"):" ")." ".implode($p[1]).($R!=""?$p[2]:""):"DROP ".idf_escape($p[0]));$c=array_merge($c,$xc);$Rf=($jb!==null?" COMMENT=".q($jb):"").($Vb?" ENGINE=".q($Vb):"").($d?" COLLATE ".q($d):"").($Ga!=""?" AUTO_INCREMENT=$Ga":"");if($R=="")return
queries("CREATE TABLE ".table($B)." (\n".implode(",\n",$c)."\n)$Rf$De");if($R!=$B)$c[]="RENAME TO ".table($B);if($Rf)$c[]=ltrim($Rf);return($c||$De?queries("ALTER TABLE ".table($R)."\n".implode(",\n",$c).$De):true);}function
alter_indexes($R,$c){foreach($c
...
drop_tables($T){return
queries("DROP TABLE ".implode(", ",array_map('table',$T)));}function
move_tables($T,$Xg,$dg){$ff=array();foreach(array_merge($T,$Xg)as$R)$ff[]=table($R)." TO ".idf_escape($dg).".".table($R);return
queries("RENAME TABLE ".implode(", ",$ff));}function
copy_tables($T,$Xg,$dg){queries("SET sql_mode = 'NO_AUTO_VALUE_ON_ZERO'");foreach($T
as$R){$B=($dg==DB?table("copy_$R"):idf_escape($dg).".".table($R));if(($_POST["overwrite"]&&!queries("\nDROP TABLE IF EXISTS $B"))||!queries("CREATE TABLE $B LIKE ".table($R))||!queries("INSERT INTO $B SELECT * FROM ".table($R)))return
false;foreach(get_rows("SHOW TRIGGERS LIKE ".q(addcslashes($R,"%_\\")))as$K){$zg=$K["Trigger"];if(!queries("CREATE TRIGGER ".($dg==DB?idf_escape("copy_$zg"):idf_escape($dg).".".idf_escape($zg))." $K[Timing] $K[Event] ON $B FOR EACH ROW\n$K[Statement];"))return
false;}}foreach($Xg
as$R){$B=($dg==DB?table("copy_$R"):idf_escape($dg).".".table($R));$Wg=view($R);if(($_POST["overwrite"]&&!queries("DROP VIEW IF EXISTS $B"))||!queries("CREATE VIEW $B AS $Wg[select]"))return
false;}return
true;}function
trigger($B){if($B=="")return
...
routine($B,$U){global$h,$Wb,$jd,$Dg;$xa=array("bool","boolean","integer","double precision","real","dec","numeric","fixed","national char","national varchar");$Kf="(?:\\s|/\\*[\s\S]*?\\*/|(?:#|-- )[^\n]*\n?|--\r?\n)";$Cg="((".implode("|",array_merge(array_keys($Dg),$xa)).")\\b(?:\\s*\\(((?:[^'\")]|$Wb)++)\\))?\\s*(zerofill\\s*)?(unsigned(?:\\s+zerofill)?)?)(?:\\s*(?:CHARSET|CHARACTER\\s+SET)\\s*['\"]?([^'\"\\s,]+)['\"]?)?";$Ge="$Kf*(".($U=="FUNCTION"?"":$jd).")?\\s*(?:`((?:[^`]|``)*)`\\s*|\\b(\\S+)\\s+)$Cg";$sb=$h->result("SHOW CREATE $U ".idf_escape($B),2);preg_match("~\\(((?:$Ge\\s*,?)*)\\)\\s*".($U=="FUNCTION"?"RETURNS\\s+$Cg\\s+":"")."(.*)~is",$sb,$A);$q=array();preg_match_all("~$Ge\\s*,?~is",$A[1],$Od,PREG_SET_ORDER);foreach($Od
as$Ae){$B=str_replace("``","`",$Ae[2]).$Ae[3];$q[]=array("field"=>$B,"type"=>strtolower($Ae[5]),"length"=>preg_replace_callback("~$Wb~s",'normalize_enum',$Ae[6]),"unsigned"=>strtolower(preg_replace('~\s+~',' ',trim("$Ae[8] $Ae[7]"))),"null"=>1,"full_type"=>$Ae[4],"inout"=>strtoupper($Ae[1]),"collation"=>strtolower($Ae[9]),);}if($U!="FUNCTION")return
...
set_schema($qf){return
true;}function
create_sql($R,$Ga,$Uf){global$h;$J=$h->result("SHOW CREATE TABLE ".table($R),1);if(!$Ga)$J=preg_replace('~ AUTO_INCREMENT=\d+~','',$J);return$J;}function
truncate_sql($R){return"TRUNCATE ".table($R);}function
use_sql($k){return"USE ".idf_escape($k);}function
...
fieldName($p,$ue=0){return
h(preg_replace('~\s+\[.*\]$~','',($p["comment"]!=""?$p["comment"]:$p["field"])));}function
selectLinks($Zf,$P=""){$a=$Zf["Name"];if($P!==null)echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($a).$P).'">'.lang(39)."</a>\n";}function
foreignKeys($R){return
foreign_keys($R);}function
backwardKeys($R,$Yf){$J=array();foreach(get_rows("SELECT TABLE_NAME, CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_COLUMN_NAME
...
as$y=>$X){$B=$this->tableName(table_status($y,true));if($B!=""){$sf=preg_quote($Yf);$N="(:|\\s*-)?\\s+";$J[$y]["name"]=(preg_match("(^$sf$N(.+)|^(.+?)$N$sf\$)iu",$B,$A)?$A[2].$A[3]:$B);}else
...
selectQuery($G,$Pf,$jc=false){return"<!--\n".str_replace("--","--><!-- ",$G)."\n(".format_time($Pf).")\n-->\n";}function
...
as$y=>$X)$vd[$X["col"]]=$y;$s=0;$q=fields($_GET["select"]);foreach($f
as$B=>$Cb){$p=$q[$B];if(preg_match("~enum~",$p["type"])||like_bool($p)){$y=$vd[$B];$s--;echo"<div>".h($Cb)."<input type='hidden' name='where[$s][col]' value='".h($B)."'>:",(like_bool($p)?" <select name='where[$s][val]'>".optionlist(array(""=>"",lang(42),lang(41)),$Z[$y]["val"],true)."</select>":enum_input("checkbox"," name='where[$s][val][]'",$p,(array)$Z[$y]["val"],($p["null"]?0:null))),"</div>\n";unset($f[$B]);}elseif(is_array($C=$this->_foreignKeyOptions($_GET["select"],$B))){if($q[$B]["null"])$C[0]='('.lang(7).')';$y=$vd[$B];$s--;echo"<div>".h($Cb)."<input type='hidden' name='where[$s][col]' value='".h($B)."'><input type='hidden' name='where[$s][op]' value='='>: <select name='where[$s][val]'>".optionlist($C,$Z[$y]["val"],true)."</select></div>\n";unset($f[$B]);}}$s=0;foreach($Z
...
as$y=>$v){$ue=array();foreach($v["columns"]as$X)$ue[]=$f[$X];if(count(array_filter($ue,'strlen'))>1&&$y!="PRIMARY")$ve[$y]=implode(", ",$ue);}if($ve){echo'<fieldset><legend>'.lang(46)."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$ve,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"])echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}function
...
selectSearchProcess($q,$w){$J=array();foreach((array)$_GET["where"]as$y=>$Z){$eb=$Z["col"];$pe=$Z["op"];$X=$Z["val"];if(($y<0?"":$eb).$X!=""){$lb=array();foreach(($eb!=""?array($eb=>$q[$eb]):$q)as$B=>$p){if($eb!=""||is_numeric($X)||!preg_match(number_type(),$p["type"])){$B=idf_escape($B);if($eb!=""&&$p["type"]=="enum")$lb[]=(in_array(0,$X)?"$B IS NULL OR ":"")."$B IN (".implode(", ",array_map('intval',$X)).")";else{$ig=preg_match('~char|text|enum|set~',$p["type"]);$Y=$this->processInput($p,(!$pe&&$ig&&preg_match('~^[^%]+$~',$X)?"%$X%":$X));$lb[]=$B.($Y=="NULL"?" IS".($pe==">="?" NOT":"")." $Y":(in_array($pe,$this->operators)||$pe=="="?" $pe $Y":($ig?" LIKE $Y":" IN (".str_replace(",","', '",$Y).")")));if($y<0&&$X=="0")$lb[]="$B IS NULL";}}}$J[]=($lb?"(".implode(" OR ",$lb).")":"1 = 0");}}return$J;}function
...
true;if($_POST["email"]){$xf=0;if($_POST["all"]||$_POST["check"]){$p=idf_escape($_POST["email_field"]);$Vf=$_POST["email_subject"];$Ud=$_POST["email_message"];preg_match_all('~\{\$([a-z0-9_]+)\}~i',"$Vf.$Ud",$Od);$L=get_rows("SELECT DISTINCT $p".($Od[1]?", ".implode(", ",array_map('idf_escape',array_unique($Od[1]))):"")." FROM ".table($_GET["select"])." WHERE $p IS NOT NULL AND $p != ''".($Z?" AND ".implode(" AND ",$Z):"").($_POST["all"]?"":" AND ((".implode(") OR (",array_map('where_check',(array)$_POST["check"]))."))"));$q=fields($_GET["select"]);foreach($this->rowDescriptions($L,$zc)as$K){$gf=array('{\\'=>'{');foreach($Od[1]as$X)$gf['{$'."$X}"]=$this->editVal($K[$X],$q[$X]);$Rb=$K[$_POST["email_field"]];if(is_mail($Rb)&&send_mail($Rb,strtr($Vf,$gf),strtr($Ud,$gf),$_POST["email_from"],$_FILES["email_files"]))$xf++;}}cookie("adminer_email",$_POST["email_from"]);redirect(remove_from_uri(),lang(55,$xf));}return
false;}function
selectQueryBuild($M,$Z,$Hc,$ue,$z,$D){return"";}function
messageQuery($G,$jg,$jc=false){return" <span class='time'>".@date("H:i:s")."</span><!--\n".str_replace("--","--><!-- ",$G)."\n".($jg?"($jg)\n":"")."-->";}function
...
dumpData($R,$Uf,$G){global$h;$I=$h->query($G,1);if($I){while($K=$I->fetch_assoc()){if($Uf=="table"){dump_csv(array_keys($K));$Uf="INSERT";}dump_csv($K);}}}function
dumpFilename($Wc){return
friendly_url($Wc);}function
dumpHeaders($Wc,$Zd=false){$fc="csv";header("Content-Type: text/csv; charset=utf-8");return$fc;}function
...
';if($Yd=="auth"){$rc=true;foreach((array)$_SESSION["pwds"]as$Ug=>$Bf){foreach($Bf[""]as$V=>$F){if($F!==null){if($rc){echo"<ul id='logins'>",script("mixin(qs('#logins'), {onmouseover: menuOver, onmouseout: menuOut});");$rc=false;}echo"<li><a href='".h(auth_url($Ug,"",$V))."'>".($V!=""?h($V):"<i>".lang(7)."</i>")."</a>\n";}}}}else{$this->databasesPrint($Yd);if($Yd!="db"&&$Yd!="ns"){$S=table_status('',true);if(!$S)echo"<p class='message'>".lang(9)."\n";else$this->tablesPrint($S);}}}function
...
_foreignKeyOptions($R,$e,$Y=null){global$h;if(list($dg,$t,$B)=$this->_foreignColumn(column_foreign_keys($R),$e)){$J=&$this->_values[$dg];if($J===null){$S=table_status($dg);$J=($S["Rows"]>1000?"":array(""=>"")+get_key_vals("SELECT $t, $B FROM ".table($dg)." ORDER BY 2"));}if(!$J&&$Y!==null)return$h->result("SELECT $B FROM ".table($dg)." WHERE $t = ".q($Y));return$J;}}}$b=(function_exists('adminer_object')?adminer_object():new
Adminer);function
page_header($mg,$o="",$Sa=array(),$ng=""){global$ba,$ca,$b,$Jb,$x;page_headers();if(is_ajax()&&$o){page_messages($o);exit;}$og=$mg.($ng!=""?": $ng":"");$pg=strip_tags($og.(SERVER!=""&&SERVER!="localhost"?h(" - ".SERVER):"")." - ".$b->name());echo'<!DOCTYPE html>
...
';$r=get_temp_dir()."/adminer.version";if(!$_COOKIE["adminer_version"]&&function_exists('openssl_verify')&&file_exists($r)&&filemtime($r)+86400>time()){$Vg=unserialize(file_get_contents($r));$Te="-----BEGIN PUBLIC KEY-----
...
fQIDAQAB
-----END PUBLIC KEY-----
";if(openssl_verify($Vg["version"],base64_decode($Vg["signature"]),$Te)==1)$_COOKIE["adminer_version"]=$Vg["version"];}echo'<script',nonce(),'>
...
page_messages($o){$Mg=preg_replace('~^[^?]*~','',$_SERVER["REQUEST_URI"]);$Vd=$_SESSION["messages"][$Mg];if($Vd){echo"<div class='message'>".implode("</div>\n<div class='message'>",$Vd)."</div>".script("messagesPrint();");unset($_SESSION["messages"][$Mg]);}if($o)echo"<div class='error'>$o</div>\n";}function
page_footer($Yd=""){global$b,$sg;echo'</div>
...
long2str($W,$Zg){$pf='';foreach($W
as$X)$pf.=pack('V',$X);if($Zg)return
substr($pf,0,end($W));return$pf;}function
str2long($pf,$Zg){$W=array_values(unpack('V*',str_pad($pf,4*ceil(strlen($pf)/4),"\0")));if($Zg)$W[]=strlen($pf);return$W;}function
xxtea_mx($kh,$jh,$Xf,$rd){return
int32((($kh>>5&0x7FFFFFF)^$jh<<2)+(($jh>>3&0x1FFFFFFF)^$kh<<4))^int32(($Xf^$jh)+($rd^$kh));}function
encrypt_string($Sf,$y){if($Sf=="")return"";$y=array_values(unpack("V*",pack("H*",md5($y))));$W=str2long($Sf,true);$be=count($W)-1;$kh=$W[$be];$jh=$W[0];$Ue=floor(6+52/($be+1));$Xf=0;while($Ue-->0){$Xf=int32($Xf+0x9E3779B9);$Nb=$Xf>>2&3;for($ze=0;$ze<$be;$ze++){$jh=$W[$ze+1];$ae=xxtea_mx($kh,$jh,$Xf,$y[$ze&3^$Nb]);$kh=int32($W[$ze]+$ae);$W[$ze]=$kh;}$jh=$W[0];$ae=xxtea_mx($kh,$jh,$Xf,$y[$ze&3^$Nb]);$kh=int32($W[$be]+$ae);$W[$be]=$kh;}return
long2str($W,false);}function
decrypt_string($Sf,$y){if($Sf=="")return"";if(!$y)return
false;$y=array_values(unpack("V*",pack("H*",md5($y))));$W=str2long($Sf,false);$be=count($W)-1;$kh=$W[$be];$jh=$W[0];$Ue=floor(6+52/($be+1));$Xf=int32($Ue*0x9E3779B9);while($Xf){$Nb=$Xf>>2&3;for($ze=$be;$ze>0;$ze--){$kh=$W[$ze-1];$ae=xxtea_mx($kh,$jh,$Xf,$y[$ze&3^$Nb]);$jh=int32($W[$ze]-$ae);$W[$ze]=$jh;}$kh=$W[$be];$ae=xxtea_mx($kh,$jh,$Xf,$y[$ze&3^$Nb]);$jh=int32($W[0]-$ae);$W[0]=$jh;$Xf=int32($Xf-0x9E3779B9);}return
long2str($W,true);}$h='';$Oc=$_SESSION["token"];if(!$Oc)$_SESSION["token"]=rand(1,1e6);$sg=get_token();$He=array();if($_COOKIE["adminer_permanent"]){foreach(explode(" ",$_COOKIE["adminer_permanent"])as$X){list($y)=explode(":",$X);$He[$y]=$X;}}function
add_invalid_login(){global$b;$Dc=file_open_lock(get_temp_dir()."/adminer.invalid");if(!$Dc)return;$nd=unserialize(stream_get_contents($Dc));$jg=time();if($nd){foreach($nd
as$od=>$X){if($X[0]<$jg)unset($nd[$od]);}}$md=&$nd[$b->bruteForceKey()];if(!$md)$md=array($jg+30*60,0);$md[1]++;file_write_unlock($Dc,serialize($nd));}function
check_invalid_login(){global$b;$nd=unserialize(@file_get_contents(get_temp_dir()."/adminer.invalid"));$md=$nd[$b->bruteForceKey()];$ee=($md[1]>29?$md[0]-time():0);if($ee>0)auth_error(lang(64,ceil($ee/60)));}$Ea=$_POST["auth"];if($Ea){session_regenerate_id();$Ug=$Ea["driver"];$O=$Ea["server"];$V=$Ea["username"];$F=(string)$Ea["password"];$m=$Ea["db"];set_password($Ug,$O,$V,$F);$_SESSION["db"][$Ug][$O][$V][$m]=true;if($Ea["permanent"]){$y=base64_encode($Ug)."-".base64_encode($O)."-".base64_encode($V)."-".base64_encode($m);$Qe=$b->permanentLogin(true);$He[$y]="$y:".base64_encode($Qe?encrypt_string($F,$Qe):"");cookie("adminer_permanent",implode(" ",$He));}if(count($_POST)==1||DRIVER!=$Ug||SERVER!=$O||$_GET["username"]!==$V||DB!=$m)redirect(auth_url($Ug,$O,$V,$m));}elseif($_POST["logout"]){if($Oc&&!verify_token()){page_header(lang(63),lang(65));page_footer("db");exit;}else{foreach(array("pwds","db","dbs","queries")as$y)set_session($y,null);unset_permanent();redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~','',ME),0,-1),lang(66).' '.lang(67));}}elseif($He&&!$_SESSION["pwds"]){session_regenerate_id();$Qe=$b->permanentLogin();foreach($He
as$y=>$X){list(,$ab)=explode(":",$X);list($Ug,$O,$V,$m)=array_map('base64_decode',explode("-",$y));set_password($Ug,$O,$V,decrypt_string(base64_decode($ab),$Qe));$_SESSION["db"][$Ug][$O][$V][$m]=true;}}function
unset_permanent(){global$He;foreach($He
as$y=>$X){list($Ug,$O,$V,$m)=array_map('base64_decode',explode("-",$y));if($Ug==DRIVER&&$O==SERVER&&$V==$_GET["username"]&&$m==DB)unset($He[$y]);}cookie("adminer_permanent",implode(" ",$He));}function
auth_error($o){global$b,$Oc;$Cf=session_name();if(isset($_GET["username"])){header("HTTP/1.1 403 Forbidden");if(($_COOKIE[$Cf]||$_GET[$Cf])&&!$Oc)$o=lang(68);else{restart_session();add_invalid_login();$F=get_password();if($F!==null){if($F===false)$o.='<br>'.lang(69,target_blank(),'<code>permanentLogin()</code>');set_password(DRIVER,SERVER,$_GET["username"],null);}unset_permanent();}}if(!$_COOKIE[$Cf]&&$_GET[$Cf]&&ini_bool("session.use_only_cookies"))$o=lang(70);$E=session_get_cookie_params();cookie("adminer_key",($_COOKIE["adminer_key"]?$_COOKIE["adminer_key"]:rand_string()),$E["lifetime"]);page_header(lang(37),$o,null);echo"<form action='' method='post'>\n","<div>";if(hidden_fields($_POST,array("auth")))echo"<p class='message'>".lang(71)."\n";echo"</div>\n";$b->loginForm();echo"</form>\n";page_footer("auth");exit;}if(isset($_GET["username"])&&!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);unset_permanent();page_header(lang(72),lang(73,implode(", ",$Le)),false);page_footer("auth");exit;}stop_session(true);if(isset($_GET["username"])&&is_string(get_password())){list($Uc,$Je)=explode(":",SERVER,2);if(is_numeric($Je)&&$Je<1024)auth_error(lang(74));check_invalid_login();$h=connect();$n=new
Min_Driver($h);}$Id=null;if(!is_object($h)||($Id=$b->login($_GET["username"],get_password()))!==true){$o=(is_string($h)?h($h):(is_string($Id)?$Id:lang(75)));auth_error($o.(preg_match('~^ | $~',get_password())?'<br>'.lang(76):''));}if($Ea&&$_POST["token"])$_POST["token"]=$sg;$o='';if($_POST){if(!verify_token()){$id="max_input_vars";$Sd=ini_get($id);if(extension_loaded("suhosin")){foreach(array("suhosin.request.max_vars","suhosin.post.max_vars")as$y){$X=ini_get($y);if($X&&(!$Sd||$X<$Sd)){$id=$y;$Sd=$X;}}}$o=(!$_POST["token"]&&$Sd?lang(77,"'$id'"):lang(65).' '.lang(78));}}elseif($_SERVER["REQUEST_METHOD"]=="POST"){$o=lang(79,"'post_max_size'");if(isset($_GET["sql"]))$o.=' '.lang(80);}function
email_header($Pc){return"=?UTF-8?B?".base64_encode($Pc)."?=";}function
send_mail($Rb,$Vf,$Ud,$Ec="",$pc=array()){$Xb=(DIRECTORY_SEPARATOR=="/"?"\n":"\r\n");$Ud=str_replace("\n",$Xb,wordwrap(str_replace("\r","","$Ud\n")));$Ra=uniqid("boundary");$Ba="";foreach((array)$pc["error"]as$y=>$X){if(!$X)$Ba.="--$Ra$Xb"."Content-Type: ".str_replace("\n","",$pc["type"][$y]).$Xb."Content-Disposition: attachment; filename=\"".preg_replace('~["\n]~','',$pc["name"][$y])."\"$Xb"."Content-Transfer-Encoding: base64$Xb$Xb".chunk_split(base64_encode(file_get_contents($pc["tmp_name"][$y])),76,$Xb).$Xb;}$Ma="";$Qc="Content-Type: text/plain; charset=utf-8$Xb"."Content-Transfer-Encoding: 8bit";if($Ba){$Ba.="--$Ra--$Xb";$Ma="--$Ra$Xb$Qc$Xb$Xb";$Qc="Content-Type: multipart/mixed; boundary=\"$Ra\"";}$Qc.=$Xb."MIME-Version: 1.0$Xb"."X-Mailer: Adminer Editor".($Ec?$Xb."From: ".str_replace("\n","",$Ec):"");return
mail($Rb,email_header($Vf),$Ma.$Ud.$Ba,$Qc);}function
like_bool($p){return
preg_match("~bool|(tinyint|bit)\\(1\\)~",$p["full_type"]);}$h->select_db($b->database());$me="RESTRICT|NO ACTION|CASCADE|SET NULL|SET DEFAULT";$Jb[DRIVER]=lang(37);if(isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"])$_GET["edit"]=$_GET["select"];if(isset($_GET["download"])){$a=$_GET["download"];$q=fields($a);header("Content-Type: application/octet-stream");header("Content-Disposition: attachment; filename=".friendly_url("$a-".implode("_",$_GET["where"])).".".friendly_url($_GET["field"]));$M=array(idf_escape($_GET["field"]));$I=$n->select($a,$M,array(where($_GET,$q)),$M);$K=($I?$I->fetch_row():array());echo$n->value($K[0],$q[$_GET["field"]]);exit;}elseif(isset($_GET["edit"])){$a=$_GET["edit"];$q=fields($a);$Z=(isset($_GET["select"])?($_POST["check"]&&count($_POST["check"])==1?where_check($_POST["check"][0],$q):""):where($_GET,$q));$Lg=(isset($_GET["select"])?$_POST["edit"]:$Z);foreach($q
as$B=>$p){if(!isset($p["privileges"][$Lg?"update":"insert"])||$b->fieldName($p)==""||$p["generated"])unset($q[$B]);}if($_POST&&!$o&&!isset($_GET["select"])){$Hd=$_POST["referer"];if($_POST["insert"])$Hd=($Lg?null:$_SERVER["REQUEST_URI"]);elseif(!preg_match('~^.+&select=.+$~',$Hd))$Hd=ME."select=".urlencode($a);$w=indexes($a);$Gg=unique_array($_GET["where"],$w);$We="\nWHERE $Z";if(isset($_POST["delete"]))queries_redirect($Hd,lang(81),$n->delete($a,$We,!$Gg));else{$P=array();foreach($q
as$B=>$p){$X=process_input($p);if($X!==false&&$X!==null)$P[idf_escape($B)]=$X;}if($Lg){if(!$P)redirect($Hd);queries_redirect($Hd,lang(82),$n->update($a,$P,$We,!$Gg));if(is_ajax()){page_headers();page_messages($o);exit;}}else{$I=$n->insert($a,$P);$Bd=($I?last_id():0);queries_redirect($Hd,lang(83,($Bd?" $Bd":"")),$I);}}}$K=null;if($_POST["save"])$K=(array)$_POST["fields"];elseif($Z){$M=array();foreach($q
...
as$y=>$X){if(!$Z)$K[$y]=null;$q[$y]=array("field"=>$y,"null"=>($y!=$n->primary),"auto_increment"=>($y==$n->primary));}}}edit_form($a,$q,$K,$Lg);}elseif(isset($_GET["select"])){$a=$_GET["select"];$S=table_status1($a);$w=indexes($a);$q=fields($a);$Ac=column_foreign_keys($a);$le=$S["Oid"];parse_str($_COOKIE["adminer_import"],$ta);$nf=array();$f=array();$hg=null;foreach($q
as$y=>$p){$B=$b->fieldName($p);if(isset($p["privileges"]["select"])&&$B!=""){$f[$y]=html_entity_decode(strip_tags($B),ENT_QUOTES);if(is_shortable($p))$hg=$b->selectLengthProcess();}$nf+=$p["privileges"];}list($M,$Hc)=$b->selectColumnsProcess($f,$w);$pd=count($Hc)<count($M);$Z=$b->selectSearchProcess($q,$w);$ue=$b->selectOrderProcess($q,$w);$z=$b->selectLimitProcess();if($_GET["val"]&&is_ajax()){header("Content-Type: text/plain; charset=utf-8");foreach($_GET["val"]as$Hg=>$K){$_a=convert_field($q[key($K)]);$M=array($_a?$_a:idf_escape(key($K)));$Z[]=where_check($Hg,$q);$J=$n->select($a,$M,$Z,$M);if($J)echo
reset($J->fetch_row());}exit;}$Ne=$Jg=null;foreach($w
as$v){if($v["type"]=="PRIMARY"){$Ne=array_flip($v["columns"]);$Jg=($M?$Ne:array());foreach($Jg
as$y=>$X){if(in_array(idf_escape($y),$M))unset($Jg[$y]);}break;}}if($le&&!$Ne){$Ne=$Jg=array($le=>0);$w[]=array("type"=>"PRIMARY","columns"=>array($le));}if($_POST&&!$o){$eh=$Z;if(!$_POST["all"]&&is_array($_POST["check"])){$Ya=array();foreach($_POST["check"]as$Va)$Ya[]=where_check($Va,$q);$eh[]="((".implode(") OR (",$Ya)."))";}$eh=($eh?"\nWHERE ".implode(" AND ",$eh):"");if($_POST["export"]){cookie("adminer_import","output=".urlencode($_POST["output"])."&format=".urlencode($_POST["format"]));dump_headers($a);$b->dumpTable($a,"");$Ec=($M?implode(", ",$M):"*").convert_fields($f,$q,$M)."\nFROM ".table($a);$Jc=($Hc&&$pd?"\nGROUP BY ".implode(", ",$Hc):"").($ue?"\nORDER BY ".implode(", ",$ue):"");if(!is_array($_POST["check"])||$Ne)$G="SELECT $Ec$eh$Jc";else{$Fg=array();foreach($_POST["check"]as$X)$Fg[]="(SELECT".limit($Ec,"\nWHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($X,$q).$Jc,1).")";$G=implode(" UNION ALL ",$Fg);}$b->dumpData($a,"table",$G);exit;}if(!$b->selectEmailProcess($Z,$Ac)){if($_POST["save"]||$_POST["delete"]){$I=true;$ua=0;$P=array();if(!$_POST["delete"]){foreach($f
as$B=>$X){$X=process_input($q[$B]);if($X!==null&&($_POST["clone"]||$X!==false))$P[idf_escape($B)]=($X!==false?$X:idf_escape($B));}}if($_POST["delete"]||$P){if($_POST["clone"])$G="INTO ".table($a)." (".implode(", ",array_keys($P)).")\nSELECT ".implode(", ",$P)."\nFROM ".table($a);if($_POST["all"]||($Ne&&is_array($_POST["check"]))||$pd){$I=($_POST["delete"]?$n->delete($a,$eh):($_POST["clone"]?queries("INSERT $G$eh"):$n->update($a,$P,$eh)));$ua=$h->affected_rows;}else{foreach((array)$_POST["check"]as$X){$ah="\nWHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($X,$q);$I=($_POST["delete"]?$n->delete($a,$ah,1):($_POST["clone"]?queries("INSERT".limit1($a,$G,$ah)):$n->update($a,$P,$ah,1)));if(!$I)break;$ua+=$h->affected_rows;}}}$Ud=lang(84,$ua);if($_POST["clone"]&&$I&&$ua==1){$Bd=last_id();if($Bd)$Ud=lang(83," $Bd");}queries_redirect(remove_from_uri($_POST["all"]&&$_POST["delete"]?"page":""),$Ud,$I);if(!$_POST["delete"]){edit_form($a,$q,(array)$_POST["fields"],!$_POST["clone"]);page_footer();exit;}}elseif(!$_POST["import"]){if(!$_POST["val"])$o=lang(85);else{$I=true;$ua=0;foreach($_POST["val"]as$Hg=>$K){$P=array();foreach($K
as$y=>$X){$y=bracket_escape($y,1);$P[idf_escape($y)]=(preg_match('~char|text~',$q[$y]["type"])||$X!=""?$b->processInput($q[$y],$X):"NULL");}$I=$n->update($a,$P," WHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($Hg,$q),!$pd&&!$Ne," ");if(!$I)break;$ua+=$h->affected_rows;}queries_redirect(remove_from_uri(),lang(84,$ua),$I);}}elseif(!is_string($oc=get_file("csv_file",true)))$o=upload_error($oc);elseif(!preg_match('~~u',$oc))$o=lang(86);else{cookie("adminer_import","output=".urlencode($ta["output"])."&format=".urlencode($_POST["separator"]));$I=true;$hb=array_keys($q);preg_match_all('~(?>"[^"]*"|[^"\r\n]+)+~',$oc,$Od);$ua=count($Od[0]);$n->begin();$N=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));$L=array();foreach($Od[0]as$y=>$X){preg_match_all("~((?>\"[^\"]*\")+|[^$N]*)$N~",$X.$N,$Pd);if(!$y&&!array_diff($Pd[1],$hb)){$hb=$Pd[1];$ua--;}else{$P=array();foreach($Pd[1]as$s=>$eb)$P[idf_escape($hb[$s])]=($eb==""&&$q[$hb[$s]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$eb))));$L[]=$P;}}$I=(!$L||$n->insertUpdate($a,$L,$Ne));if($I)$I=$n->commit();queries_redirect(remove_from_uri("page"),lang(87,$ua),$I);$n->rollback();}}}$ag=$b->tableName($S);if(is_ajax()){page_headers();ob_start();}else
page_header(lang(49).": $ag",$o);$P=null;if(isset($nf["insert"])||!support("table")){$P="";foreach((array)$_GET["where"]as$X){if($Ac[$X["col"]]&&count($Ac[$X["col"]])==1&&($X["op"]=="="||(!$X["op"]&&!preg_match('~[_%]~',$X["val"]))))$P.="&set".urlencode("[".bracket_escape($X["col"])."]")."=".urlencode($X["val"]);}}$b->selectLinks($S,$P);if(!$f&&support("table"))echo"<p class='error'>".lang(88).($q?".":": ".error())."\n";else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($a).'">',"</div>\n";$b->selectColumnsPrint($M,$f);$b->selectSearchPrint($Z,$f,$w);$b->selectOrderPrint($ue,$f,$w);$b->selectLimitPrint($z);$b->selectLengthPrint($hg);$b->selectActionPrint($w);echo"</form>\n";$D=$_GET["page"];if($D=="last"){$Cc=$h->result(count_rows($a,$Z,$pd,$Hc));$D=floor(max(0,$Cc-1)/$z);}$uf=$M;$Ic=$Hc;if(!$uf){$uf[]="*";$qb=convert_fields($f,$q,$M);if($qb)$uf[]=substr($qb,2);}foreach($M
as$y=>$X){$p=$q[idf_unescape($X)];if($p&&($_a=convert_field($p)))$uf[$y]="$_a AS $X";}if(!$pd&&$Jg){foreach($Jg
as$y=>$X){$uf[]=idf_escape($y);if($Ic)$Ic[]=idf_escape($y);}}$I=$n->select($a,$uf,$Z,$Ic,$ue,$z,$D,true);if(!$I)echo"<p class='error'>".error()."\n";else{if($x=="mssql"&&$D)$I->seek($z*$D);$Tb=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$L=array();while($K=$I->fetch_assoc()){if($D&&$x=="oracle")unset($K["RNUM"]);$L[]=$K;}if($_GET["page"]!="last"&&$z!=""&&$Hc&&$pd&&$x=="sql")$Cc=$h->result(" SELECT FOUND_ROWS()");if(!$L)echo"<p class='message'>".lang(12)."\n";else{$La=$b->backwardKeys($a,$ag);echo"<div class='scrollable'>","<table id='table' cellspacing='0' class='nowrap checkable'>",script("mixin(qs('#table'), {onclick: tableClick, ondblclick: partialArg(tableClick, true), onkeydown: editingKeydown});"),"<thead><tr>".(!$Hc&&$M?"":"<td><input type='checkbox' id='all-page' class='jsonly'>".script("qs('#all-page').onclick = partial(formCheck, /check/);","")." <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".lang(89)."</a>");$ce=array();$Gc=array();reset($M);$Ye=1;foreach($L[0]as$y=>$X){if(!isset($Jg[$y])){$X=$_GET["columns"][key($M)];$p=$q[$M?($X?$X["col"]:current($M)):$y];$B=($p?$b->fieldName($p,$Ye):($X["fun"]?"*":$y));if($B!=""){$Ye++;$ce[$y]=$B;$e=idf_escape($y);$Vc=remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($y);$Cb="&desc%5B0%5D=1";echo"<th>".script("mixin(qsl('th'), {onmouseover: partial(columnMouse), onmouseout: partial(columnMouse, ' hidden')});",""),'<a href="'.h($Vc.($ue[0]==$e||$ue[0]==$y||(!$ue&&$pd&&$Hc[0]==$e)?$Cb:'')).'">';echo
apply_sql_function($X["fun"],$B)."</a>";echo"<span class='column hidden'>","<a href='".h($Vc.$Cb)."' title='".lang(90)."' class='text'> â†“</a>";if(!$X["fun"]){echo'<a href="#fieldset-search" title="'.lang(44).'" class="text jsonly"> =</a>',script("qsl('a').onclick = partial(selectSearch, '".js_escape($y)."');");}echo"</span>";}$Gc[$y]=$X["fun"];next($M);}}$Ed=array();if($_GET["modify"]){foreach($L
as$K){foreach($K
as$y=>$X)$Ed[$y]=max($Ed[$y],min(40,strlen(utf8_decode($X))));}}echo($La?"<th>".lang(91):"")."</thead>\n";if(is_ajax()){if($z%2==1&&$D%2==1)odd();ob_end_clean();}foreach($b->rowDescriptions($L,$Ac)as$be=>$K){$Gg=unique_array($L[$be],$w);if(!$Gg){$Gg=array();foreach($L[$be]as$y=>$X){if(!preg_match('~^(COUNT\((\*|(DISTINCT )?`(?:[^`]|``)+`)\)|(AVG|GROUP_CONCAT|MAX|MIN|SUM)\(`(?:[^`]|``)+`\))$~',$y))$Gg[$y]=$X;}}$Hg="";foreach($Gg
as$y=>$X){if(($x=="sql"||$x=="pgsql")&&preg_match('~char|text|enum|set~',$q[$y]["type"])&&strlen($X)>64){$y=(strpos($y,'(')?$y:idf_escape($y));$y="MD5(".($x!='sql'||preg_match("~^utf8~",$q[$y]["collation"])?$y:"CONVERT($y USING ".charset($h).")").")";$X=md5($X);}$Hg.="&".($X!==null?urlencode("where[".bracket_escape($y)."]")."=".urlencode($X):"null%5B%5D=".urlencode($y));}echo"<tr".odd().">".(!$Hc&&$M?"":"<td>".checkbox("check[]",substr($Hg,1),in_array(substr($Hg,1),(array)$_POST["check"])).($pd||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($a).$Hg)."' class='edit'>".lang(92)."</a>"));foreach($K
as$y=>$X){if(isset($ce[$y])){$p=$q[$y];$X=$n->value($X,$p);if($X!=""&&(!isset($Tb[$y])||$Tb[$y]!=""))$Tb[$y]=(is_mail($X)?$ce[$y]:"");$_="";if(preg_match('~blob|bytea|raw|file~',$p["type"])&&$X!="")$_=ME.'download='.urlencode($a).'&field='.urlencode($y).$Hg;if(!$_&&$X!==null){foreach((array)$Ac[$y]as$_c){if(count($Ac[$y])==1||end($_c["source"])==$y){$_="";foreach($_c["source"]as$s=>$Jf)$_.=where_link($s,$_c["target"][$s],$L[$be][$Jf]);$_=($_c["db"]!=""?preg_replace('~([?&]db=)[^&]+~','\1'.urlencode($_c["db"]),ME):ME).'select='.urlencode($_c["table"]).$_;if($_c["ns"])$_=preg_replace('~([?&]ns=)[^&]+~','\1'.urlencode($_c["ns"]),$_);if(count($_c["source"])==1)break;}}}if($y=="COUNT(*)"){$_=ME."select=".urlencode($a);$s=0;foreach((array)$_GET["where"]as$W){if(!array_key_exists($W["col"],$Gg))$_.=where_link($s++,$W["col"],$W["val"],$W["op"]);}foreach($Gg
as$rd=>$W)$_.=where_link($s++,$rd,$W);}$X=select_value($X,$_,$p,$hg);$t=h("val[$Hg][".bracket_escape($y)."]");$Y=$_POST["val"][$Hg][bracket_escape($y)];$Pb=!is_array($K[$y])&&is_utf8($X)&&$L[$be][$y]==$K[$y]&&!$Gc[$y];$gg=preg_match('~text|lob~',$p["type"]);if(($_GET["modify"]&&$Pb)||$Y!==null){$Lc=h($Y!==null?$Y:$K[$y]);echo"<td>".($gg?"<textarea name='$t' cols='30' rows='".(substr_count($K[$y],"\n")+1)."'>$Lc</textarea>":"<input name='$t' value='$Lc' size='$Ed[$y]'>");}else{$Jd=strpos($X,"<i>â€¦</i>");echo"<td id='$t' data-text='".($Jd?2:($gg?1:0))."'".($Pb?"":" data-warning='".h(lang(93))."'").">$X</td>";}}}if($La)echo"<td>";$b->backwardKeysPrint($La,$L[$be]);echo"</tr>\n";}if(is_ajax())exit;echo"</table>\n","</div>\n";}if(!is_ajax()){if($L||$D){$bc=true;if($_GET["page"]!="last"){if($z==""||(count($L)<$z&&($L||!$D)))$Cc=($D?$D*$z:0)+count($L);elseif($x!="sql"||!$pd){$Cc=($pd?false:found_rows($S,$Z));if($Cc<max(1e4,2*($D+1)*$z))$Cc=reset(slow_query(count_rows($a,$Z,$pd,$Hc)));else$bc=false;}}$_e=($z!=""&&($Cc===false||$Cc>$z||$D));if($_e){echo(($Cc===false?count($L)+1:$Cc-$D*$z)>$z?'<p><a href="'.h(remove_from_uri("page")."&page=".($D+1)).'" class="loadmore">'.lang(94).'</a>'.script("qsl('a').onclick = partial(selectLoadMore, ".(+$z).", '".lang(95)."â€¦');",""):''),"\n";}}echo"<div class='footer'><div>\n";if($L||$D){if($_e){$Qd=($Cc===false?$D+(count($L)>=$z?2:1):floor(($Cc-1)/$z));echo"<fieldset>";if($x!="simpledb"){echo"<legend><a href='".h(remove_from_uri("page"))."'>".lang(96)."</a></legend>",script("qsl('a').onclick = function () { pageClick(this.href, +prompt('".lang(96)."', '".($D+1)."')); return false; };"),pagination(0,$D).($D>5?" â€¦":"");for($s=max(1,$D-4);$s<min($Qd,$D+5);$s++)echo
pagination($s,$D);if($Qd>0){echo($D+5<$Qd?" â€¦":""),($bc&&$Cc!==false?pagination($Qd,$D):" <a href='".h(remove_from_uri("page")."&page=last")."' title='~$Qd'>".lang(97)."</a>");}}else{echo"<legend>".lang(96)."</legend>",pagination(0,$D).($D>1?" â€¦":""),($D?pagination($D,$D):""),($Qd>$D?pagination($D+1,$D).($Qd>$D+1?" â€¦":""):"");}echo"</fieldset>\n";}echo"<fieldset>","<legend>".lang(98)."</legend>";$Hb=($bc?"":"~ ").$Cc;echo
